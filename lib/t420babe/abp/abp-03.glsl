...
mat2 abp_03_scale(vec2 _abp_03_scale) {
  return mat2(_abp_03_scale.x, 0.0, 0.0, _abp_03_scale.y);
}

// Overload for scalar 
mat2 abp_03_scale(float _abp_03_scale) {
  return mat2(_abp_03_scale, 0.0, 0.0, _abp_03_scale);
}

float spinning_cirlces_illusion(vec2 st, float time, float time_abp_03_scale, float offset, float smoothing_factor){
 ...
 // Relative to translated coord sys (dividing with numbers with various common factors leads to more symmetric or chaotic ring resonance)
    ...
    // Translate coords - remember origin in in the btm left
    st -= center;
    st = rotate2d(time * time_abp_03_scale) * st;
    // circle += plot(st, r);
...
void abp_03(vec3 p3, float time, peakamp audio) {
  vec2 st = p3.xy;
  st += 0.5;
  vec2 xy = st;
