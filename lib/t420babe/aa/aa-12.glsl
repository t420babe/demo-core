...
// Rainbow scales
vec3 aa_12_1(vec3 p3, float time, vec2 u_res, peakamp audio) {
  vec2 pos = p3.xy;
  pos = aa_rb_pos(pos, u_res, time);
  pos.y -= 0.5;
  vec3 color = aa_00_1(p3, time * 0.1, audio);
  color =  vec3(color.x * pos.y, color.x, color.x);
  return vec3(color.x * pos.y, color.x, color.x);
}

void aa_12(vec3 p3, float time, vec2 u_res, peakamp audio) {
  vec3 color = aa_12_1(p3, time, u_res, audio);
