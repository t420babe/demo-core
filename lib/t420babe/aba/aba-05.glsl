...
vec3 aba_05_alternate(in vec2 pos, vec3 color, float time, peakamp audio) {
...
void aba_05(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  vec3 color = vec3(1.0);
  float mul = 2.0;
  audio.lowpass   *= mul;
  ...
  color = aba_05_alternate(pos, color, time, audio);
  color = 1.0 - color;
