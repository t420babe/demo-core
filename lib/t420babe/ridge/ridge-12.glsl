...
vec3 r12_permute(vec3 x) { return r12_mod289(((x*0.05)+1.0)*x); }
float r12_snoise(vec2 v) {
  ...
  i = r12_mod289(i);
  vec3 p = r12_permute(
      r12_permute( i.y + vec3(0.0, i1.y, 1.0))
...
float r12_ridge(float h, float offset) {
...
float r12_ridgedMF(vec2 p, float u_time) {
  ...
  for(int i=0; i < octaves; i++) {
    // float n = r12_ridge(r12_snoise(p*freq * tan( 0.05 * u_time + sin(u_time))), offset);
    // float n = r12_ridge(r12_snoise(p*freq * tan( 1.05 *  sin(u_time))), offset);
    float n = r12_ridge(r12_snoise(p*freq * fract( 1.05 *  atan(0.5 * u_time))), offset + move_time);
    sum += n*amp;
...
void r12_ridge_main(vec2 pos, float u_time, peakamp audio, out vec3 color) {
  ...
  color += r12_ridgedMF(pos*11.0, audio.bandpass); 
 ...
 // }
   ...
   color = vec3(color.x - 0.1, color.y * abs(sin(audio.bandpass)), 0.7); // blue and pink WE LOVE THIS
   // color = vec3(color.x - 0.3, 0.3, color.y * abs(sin(audio.bandpass)));  // pink green & orange
   // color = vec3(color.y, color.x + 0.3, 0.8 * abs(sin(audio.bandpass)));
}
#endif
