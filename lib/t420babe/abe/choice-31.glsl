...
float choice_31_map(vec3 pos, float time){
...
vec3 choice_31(vec2 pos, float time, peakamp audio) {
  // 365.0
  ...
  for(int i = 0; i <= 1; i++)	{
    ...
    float rz = choice_31_map(pos, time);
    float dim = 1.0;
    float f = clamp( ( rz - choice_31_map(pos + 0.5, time * 1.5) ) * dim, 0.5, 5.0 );
    float r_mul = 1.0;
    float g_mul = 1.0;
    float b_mul = 1.0;
