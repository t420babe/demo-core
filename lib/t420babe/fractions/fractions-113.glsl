...
#ifndef T4B_FRACTIONS_113
#define T4B_FRACTIONS_113
...
float fractions_113_map(vec3 p3, float time) {
  ...
  float x2 = tan(p3.x * (q.y + (q.x / q.y) ) ) * 5.5;
  return x0 * x1 + x2 * 5.0;
}

void fractions_113(vec3 p3, float time, peakamp audio) {
  // Add 10s to avoid solid black screen @ t=0
  time += 1000.0;
  // time *= abs(sin(p3.x)) * 2.5;
  vec3 color = vec3(1.0);
  audio.lowpass *= 2.0;
  audio.highpass *= 2.0;
  audio.bandpass *= 2.0;
  ...
  float rz = fractions_113_map(p3, time);
  ...
  float f =  ( rz - fractions_113_map(p3 * 1.0, wrap_time(time, 10.0)) ) ;
  ...
  color.g *= sin(time) * 5.0;
  // color = abs(0.7 * audio.notch) - color;
