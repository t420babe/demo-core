...
float abf_111_map(vec3 p3, float time) {
...
vec3 abf_111_fake_fluid(vec3 p3, float time, peakamp audio) {
	vec2 uv = p3.xy;
...
void abf_111(vec3 p3, float time, peakamp audio) {
  ...
  float f =  ( rz / abf_111_map(p3, wrap_time(time, 10.0)) ) ;
  ...
  color *= abf_111_fake_fluid(p3_raw * 0.5, time_raw, audio);
