// #chill 
// The Best Rapper Alive by DJ Streaks
#ifndef T4B_ABH_36
#define T4B_ABH_36
...
#endif
mat2 abh_36_rotate2d(float a){
  float c=cos(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float abh_36_blob(vec3 p3, float time){
    p3.xz *= rotate2d(time * 0.4);
    p3.xy*= rotate2d(time * 0.3);
    vec3 q = p3 * 2.0 + time;
    float x0 = length( p3 + vec3( sin(time * 0.7) ) );
    float x1 = tan( length(p3) + 1.0 );
    float x2 = atan( q.x + tan( q.z + length(q) ) ) * 0.5 - 1.0;
    return x0 * x1 + x2;
}


void abh_36(vec3 p3, float time, peakamp audio) {
  // audio.lowpass = 0.05;
  // audio.highpass = 0.05;
  // audio.bandpass = 0.05;
  // audio.notch = 0.05;
  ...
  // p3.x -= 0.5;
  float rz = abh_36_blob(1.0 * p3, time);
  float f = ( rz / abh_36_blob(p3, y) ) * 10.0;
