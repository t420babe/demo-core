...
#endif
mat2 abh_57_rotate2d(float a){
  float c=cos(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float abh_57_blob(vec3 p3, float time){
  p3 = p3.yxz;
...
void abh_57(vec3 p3, float time, peakamp audio) {
  time += 1000.0;
  vec3 color = vec3(1.0);
  // p3.y *= 0.30;
  ...
  // p3.x -= 0.5;
  float rz = abh_57_blob(p3, time);
  float f = ( rz * abh_57_blob(p3, y) ) * 1.0;
