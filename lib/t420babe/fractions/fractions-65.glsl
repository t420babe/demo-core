// The Job by Kousto, Aroop Roy
#ifndef T4B_FRACTIONS_65
#define T4B_FRACTIONS_65
...
float fractions_65_map(vec3 p3, float time) {
...
void fractions_65(vec3 p3, float time, peakamp audio) {
  ...
  float rz = fractions_65_map(p3, time);
  ...
  float f =  ( rz - fractions_65_map(p3 * 1.0, wrap_time(time, 10.0)) ) ;
  ...
  // color.r *= wrap_time(time, 1.0) * 2.0;
  color.r *= audio.notch * 3.0;
  // color.r += 10.0 * audio.highpass;
  color.g *= 3.0 * audio.bandpass;
