// #effect #shadershoot
#ifndef T420BABE_GAZE_18
#define T420BABE_GAZE_18
...
vec3 gaze_18(vec2 pos, float u_time, peakamp audio) {
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.5;
  audio.highpass  *= 1.0;
  audio.bandpass  *= 1.5;
  ...
  float tri_n = triangle_web_0(atan(pos), audio, u_time);

  vec3 n_color;
  float n = cellular_2d(5.5 * pos * sin(u_time * 0.5), u_time, audio, n_color);
  ...
  color -= vec3(sharp(tri_n)) / 1.3;
  color.g *= (sharp(tri_n)) * 2.0;
