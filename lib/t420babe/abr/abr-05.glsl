...
vec3 color_2(vec3 p, vec3 col, float time) {
    col = max(col, 1.2 * vec3(0.5 + 0.5 * cos(2.0 * p.y)));
    col *= abs(1.05 * p.y);
    col *= 0.5 + 0.5 * cos(length(p) + sin(time));
    float a = 2.0;
    float b = 2.5 * length(p) - sin(time);
...
void abr_05(vec3 p3, float time, peakamp audio) {
  // vec2 uv = p3.xy * 2.0 * (sin(time * 0.1) + 1.0);
  // vec2 uv = p3.xy * 1.0 * (sin(time * 0.1) + 2.0);
  // vec2 uv = p3.xy * 1.0 * (sin(time * 0.1) + 1.5);
  vec2 uv = p3.xy * 2.0 * (sin(time * 0.1) + 0.8);

  // rate
  float alt_time = time;

  // float height = 0.0;
  float height = sin(alt_time);
  float dist = 5.0;
  vec3 ro = vec3(dist * sin(alt_time), height * cos(time), dist * cos(alt_time));
  ro.yz *= rotate2d(-uv.y * PI + 1.0);
  ro.xz *= rotate2d(-uv.x * TAU);
  ...
  if(d < MAX_DIST) {
    ...
    // col = color_0(p, col, n, time);
    // col = color_1(p, col, n, rd, time);
