...
float r9_snoise(vec2 v) {
  ...
  i = r9_mod289(i);
  vec3 p = r9_permute(
      r9_permute( i.y + vec3(0.0, i1.y, 1.0))
...
float r9_ridge(float h, float offset) {
...
float r9_ridgedMF(vec2 p, float u_time) {
...
void r9_ridge_main(vec2 pos, float u_time, peakamp audio, out vec3 color) {
  ...
  color += r9_ridgedMF(pos*11.0, audio.bandpass * pos.x); 

  // color = vec3(0.8, color.y, color.x * abs(sin(u_time)));  // red and yellow
  color = vec3(0.8, color.x, 0.4 * abs(sin(u_time)));    // yellow and reddish magenta FIRE
