...
#endif
mat2 abh_73_blob_m(float a){
  float c=tan(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float abh_73_blob(vec3 p3, float time){
    p3.xz *= abh_73_blob_m(time * 0.4);
    p3.xy*= abh_73_blob_m(time * 0.3);
    vec3 q = p3 * 2.0 + time;
    float x0 = length( p3 );
...
void abh_73(vec3 p3, float time, peakamp audio) {
  ...
  float rz = abh_73_blob(p3, time);
  float f = ( rz * abh_73_blob(p3, y) ) * 1.0;
