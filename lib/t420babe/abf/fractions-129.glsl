// #katie #chill #fav3
#ifndef T4B_FRACTIONS_129
#define T4B_FRACTIONS_129
...
void fractions_129(vec3 p3, float time, peakamp audio) {
  vec3 color = vec3(1.0);
  // p3 *= time * 0.5;
  // p3 *= sin_in_out((wrap_time(time * 0.1, 5.0))) + 0.0;
  p3 *= (wrap_time(time * 0.1, 5.0));
  // p3 *= 3.0;
  // p3.y += 0.5;
  p3.x *= 0.1;
  // p3.y *= 5.0;
  ...
  // float m = f_129_plot(vec2(p3.x, p3.y), y) * 1.0;
