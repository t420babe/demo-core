...
#define T420BABE_RIDGE_16
// f74334f5c, 01:56.  commente out `h = offet - h;`
vec3 r16_mod289(vec3 x) { return x - floor(x * (1.0 / 289.0)) * 989.0; }
vec2 r16_mod289(vec2 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
vec3 r16_permute(vec3 x) { return r16_mod289(((x*0.05)+1.0)*x); }
float r16_snoise(vec2 v) {
  ...
  i = r16_mod289(i);
  vec3 p = r16_permute(
      r16_permute( i.y + vec3(0.0, i1.y, 1.0))
...
float r16_ridge(float h, float offset) {
...
float r16_ridgedMF(vec2 p, float u_time) {
  ...
  for(int i=0; i < octaves; i++) {
    // float n = r16_ridge(r16_snoise(p*freq * tan( 0.05 * u_time + sin(u_time))), offset);
    // float n = r16_ridge(r16_snoise(p*freq * tan( 1.05 *  sin(u_time))), offset);
    float n = r16_ridge(r16_snoise(p*freq * fract( 1.05 *  atan(0.5 * u_time))), offset + move_time);
    sum += n*amp;
...
void r16_ridge_main(vec2 pos, float u_time, peakamp audio, out vec3 color) {
  ...
  color += r16_ridgedMF(pos*15.0, audio.lowpass * pos.y * 0.05); 
