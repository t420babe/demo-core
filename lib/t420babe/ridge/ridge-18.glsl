...
#define T420BABE_RIDGE_18
// b7446d9, 02:09 i cant help myself i just love tan
vec3 r18_mod289(vec3 x) { return x - floor(x * (1.0 / 289.0)) * 989.0; }
vec2 r18_mod289(vec2 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
vec3 r18_permute(vec3 x) { return r18_mod289(((x*0.05)+1.0)*x); }
float r18_snoise(vec2 v) {
  ...
  i = r18_mod289(i);
  vec3 p = r18_permute(
      r18_permute( i.y + vec3(0.0, i1.y, 1.0))
...
float r18_ridge(float h, float offset) {
...
float r18_ridgedMF(vec2 p, float u_time) {
  ...
  for(int i=0; i < octaves; i++) {
    // float n = r18_ridge(r18_snoise(p*freq * tan( 0.05 * u_time + sin(u_time))), offset);
    // float n = r18_ridge(r18_snoise(p*freq * tan( 1.05 *  sin(u_time))), offset);
    float n = r18_ridge(r18_snoise(p*freq * fract( 1.05 *  atan(0.5 * u_time))), offset + move_time);
    sum += n*amp;
...
void r18_ridge_main(vec2 pos, float u_time, peakamp audio, out vec3 color) {
  ...
  color += r18_ridgedMF(pos*5.0, audio.lowpass * pos.y * 0.05); 
