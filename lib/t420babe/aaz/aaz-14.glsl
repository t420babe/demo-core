...
float aaz_14_spiral(vec2 st, float t) {
    float r = dot(st.yx, st.yx) * 0.5;
    float a = atan(st.y,st.x)  * 0.5;
    return abs(((sin(r * t)   / r)));
}

float aaz_14_flower(vec2 st, int N, peakamp audio) {
...
void aaz_14(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.5;
  audio.highpass  *= 1.5;
  ...
  pos *= 3.0;
  // pos.y += 1.5;
  color /= abs(sin(time * 0.1) + 2.0) / aaz_14_spiral(abs(tan(pos.yx) / atan(pos.xy)) * 4.5 * abs(1.0 * audio.bandpass), 0.1 * wrap_time(time, 10.0) + 10.0);
  color *= abs(cos(time * 0.1) + 1.0) / aaz_14_spiral(abs(fract(pos.yx) / tan(st.xy)) * 4.5 * abs(1.0 * audio.bandpass), 0.1 * wrap_time(time, 10.0) + 10.0);
  ...
  float flower = aaz_14_flower(pos / 2.0, 10, audio);
  color *= flower;
