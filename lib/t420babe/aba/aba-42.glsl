...
vec3 aba_42_alternate(in vec2 pos, vec3 color, float time, peakamp audio) {
  ...
  pos = rotate2d(fract(time * 0.1) * tan(time * 0.1) *0.14) * pos.yx;
...
void aba_42(vec3 p3, float time, peakamp audio) {
  ...
  color = aba_42_alternate(pos, color, time, audio);
