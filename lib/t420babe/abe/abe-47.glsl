...
float abe_47_map(vec3 pos, float time){
...
void abe_47(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.0;
  audio.highpass  *= 1.0;
  ...
  for(int i = 0; i <= 10; i++)	{
    ...
    float rz = abe_47_map(pos, time);
    float f = clamp( ( rz - abe_47_map(pos + 0.5, time) ) * 0.5, 0.1, 1.0 );
