...
#ifndef T420BABE_CHAIN_02
#define T420BABE_CHAIN_02
...
void chain_02(vec2 pos, float u_time, peakamp audio, inout vec3 color) {
  audio.lowpass   *= 1.0;
  audio.highpass  *= 1.0;
  audio.bandpass  *= 1.0;
  audio.notch     *= 2.0;
  ...
  vec3 rect_color = vec3(sharp(rect_sdf(pos, vec2(15.0, 10.0))));
  clouds((pos + vec2(0.35)) * 1.5, u_time, audio, color);

  color -= rect_color;

  color.r += rect_color.b;
