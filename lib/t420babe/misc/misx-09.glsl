// #effect #effectmisx_09_shap #fav5 #shadershoot #corey
#ifndef T420BABE_MISX_09
#define T420BABE_MISX_09
...
float misx_09_shap(vec2 pos, float radius, float u_time, peakamp audio) {
...
float misx_09_shap_border(vec2 pos, float radius, float width, float u_time, peakamp audio) {
  return misx_09_shap(pos, radius, u_time, audio) - misx_09_shap(pos, radius - width, u_time, audio);
}

vec3  misx_09(vec2 pos, float u_time, peakamp audio) {
  vec3 color = vec3(1.0);
  audio.lowpass   *= 0.1;
  audio.highpass  *= 1.0;
  audio.bandpass  *= 2.0;
  ...
  // misx_09_shap_color_border(pos, 1.0, 0.10, u_time, audio, color);

  // Color 0
  // color.b += audio.lowpass * 2.0;
  color *= misx_09_shap_border(pos, 1.0, 0.10, u_time, audio);
  color.g *= audio.lowpass * 2.0;

  // // Color 0
  // color.g += audio.lowpass * 2.0;
  // color *= misx_09_shap_border(pos, 1.0, 0.01, u_time, audio);
  // color.g *= audio.lowpass * 2.0;
