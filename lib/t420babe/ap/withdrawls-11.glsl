#ifndef T420BABE_WITHDRAWLS_11
#define T420BABE_WITHDRAWLS_11
...
float contour_lines(vec2 _pos) {
  ...
  float n = simplex3( vec3((_pos * scale), (u_time * 0.02))) / 35.0;
...
void green_concentric(vec2 pos, float u_time, peakamp audio, out vec3 color) {
  ...
  color = vec3(pct * color + pct + color.rbg);
...
// Needs large u_time
void withdrawls_11(vec2 pos, float u_time, peakamp audio, inout vec3 color) {
  if (u_time < 100000.0) {
    u_time += 10000.0;
  }
  vec3 c_brick = vec3(abs(audio.notch), abs(cos(u_time - 0.5) + 0.1) + 0.3, abs(sin(u_time - 0.5) + 0.1) +0.3);
  color = color.rbr;
  color = c_brick;
  vec2 _pos = pos;
  _pos *= 2.0;
  // _pos.x -= 5.0;
  vec3 circ_color = vec3(1.0);
  green_concentric(pos, u_time, audio, circ_color);
  color = circ_color;
