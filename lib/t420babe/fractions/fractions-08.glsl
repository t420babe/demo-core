...
#ifndef T4B_FRACTIONS_08
#define T4B_FRACTIONS_08
...
float f_08_plot(vec2 p2, float m) {
  return smoothstep(m - 0.15, m, p2.y) - smoothstep(m, m + 0.15, p2.y);
}

void fractions_08(vec3 p3, float time, peakamp audio) {
  vec3 color = vec3(1.0);
  // p3 *= time * 0.5;
  p3 *= 5.0;
  // p3 *= 3.0;
  ...
  // float m = f_08_plot(vec2(p3.x, p3.y), y) * 1.0;
  float m = plot(vec2(p3.x, p3.y), y, 1.50) * 5.0;
