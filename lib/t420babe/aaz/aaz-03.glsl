...
vec2 aaz_03_cellular2x2x2(vec3 P) {
...
float aaz_03_spiral(vec2 st, float t) {
    float r = dot(st.yx, st.yx);
    float a = atan(st.y,st.x);
    return abs(((fract(r) * t / 1.0 * 1.000)));
}

void aaz_03(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  vec3 color = vec3(1.0);
  audio.lowpass   *= 0.5;
  audio.highpass  *= 0.5;
  ...
  st *= 25.0 * abs(sin(time * 0.01));
	vec2 F = aaz_03_cellular2x2x2(vec3(st * 1.0, time));
	float n = smoothstep(0.0, abs(sin(time * 0.05)) + 1.0, F.x) / ( abs(audio.notch * 1.0));
  // n = step(n, sin(pos.x));
  color = vec3(n);
  color -= aaz_03_spiral(pos.yx * 7.0 * abs(audio.bandpass * 0.5), wrap_time(time, 10.0) + 10.0);
