...
vec3 aba_03_alternate(in vec2 pos, vec3 color, float time, peakamp audio) {
  pos = pos.yx * 1.0;
...
void aba_03(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  vec3 color = vec3(1.0);
  float mul = 2.5;
  audio.lowpass   *= mul;
  ...
  color = aba_03_alternate(pos, color, time, audio);
  color = 1.0 - color;
