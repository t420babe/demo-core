...
#define T420BABE_IN_SEARCH_OF_17
...
vec2 iso_17_cellular2x2x2(vec3 P) {
...
float iso_17_spiral(vec2 st, float t) {
...
vec3 iso_17(vec2 pos, float u_time, peakamp audio) {
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.0;
  audio.highpass  *= 1.0;
  audio.bandpass  *= 1.0;
  ...
  st *= 25.0 * abs(sin(u_time * 0.01));
	vec2 F = iso_17_cellular2x2x2(vec3(st * 1.0, u_time));
	float n = smoothstep(0.0, abs(sin(u_time * 0.05)) + 1.0, F.x) / ( abs(audio.notch * 1.0));
  // n = step(n, sin(pos.x));
  color = vec3(n);
  color -= iso_17_spiral(pos.yx * 7.0 * abs(audio.bandpass * 0.5), wrap_time(u_time, 10.0) + 10.0);
