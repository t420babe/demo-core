...
void abq_26(vec3 p3, float time, peakamp audio) {
  p3 *= 2.0;
  vec2 uv = cos(p3.xy);
  ...
  // vec3 ro = vec3(cos(time * 0.26) * 3.0, 2, sin(time * 0.26) * 3.0);
  // vec3 ro = vec3(1.0, sin(time), 1.0);
  vec2 ro_uv = rotate2d(time) * uv;
  vec3 ro = vec3(sin(time * 0.26) * 2.0, cos(time * 0.26) * 2.0, 2.0);
  ...
  // float t = uv.x < sin(0.26 * time) + cos(0.26 * time) ? raymarch(ro, rd) : hraymarch(ro, rd);
