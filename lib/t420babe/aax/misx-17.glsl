// #effect #effectmisx_17_shape #fav4 #shadershoot #corey #needsvid
// ENG98 - Extended Mix by Monki
#ifndef T420BABE_MISX_17
#define T420BABE_MISX_17
...
float misx_17_shape(vec2 pos, float radius, float u_time, peakamp audio) {
...
float misx_17_shape_border(vec2 pos, float radius, float width, float u_time, peakamp audio) {
  return misx_17_shape(pos, radius, u_time, audio) - misx_17_shape(pos, radius - width, u_time, audio);
}

vec3 misx_17(vec2 pos, float u_time, peakamp audio) {
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.0;
  audio.highpass  *= 1.0;
  audio.bandpass  *= 1.0;
  ...
  // misx_17_shape_color_border(pos, 1.0, 0.10, u_time, audio, color);

  // Color 0
  color /= misx_17_shape_border(pos, 1.0, 0.10, u_time, audio);
  color.r /= sin(audio.lowpass * 0.5);
  color.b *= audio.lowpass * 1.0;

  // // Color 0
  // color.b += audio.lowpass * 1.0;
  // color *= misx_17_shape_border(pos, 3.0, 0.01, u_time, audio);
