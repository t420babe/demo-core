// #effect #effectmisx_08_shape #fav4 #shadershoot #show
#ifndef T420BABE_MISX_08
#define T420BABE_MISX_08
...
float misx_08_shape(vec2 pos, float radius, float u_time, peakamp audio) {
...
float misx_08_shape_border(vec2 pos, float radius, float width, float u_time, peakamp audio) {
  return misx_08_shape(pos, radius, u_time, audio) - misx_08_shape(pos, radius - width, u_time, audio);
}

vec3 misx_08(vec2 pos, float u_time, peakamp audio) {
  vec3 color = vec3(1.0);
  audio.lowpass   *= 0.1;
  audio.highpass  *= 1.0;
  audio.bandpass  *= 2.0;
  ...
  // misx_08_shape_color_border(pos, 1.0, 0.10, u_time, audio, color);

  // Color 0
  color.b += audio.lowpass * 2.0;
  color *= misx_08_shape_border(pos, 1.0, 0.10, u_time, audio);
  color.g *= audio.lowpass * 2.0;

  // Color 0
  // color.g += audio.lowpass * 2.0;
  // color *= misx_08_shape_border(pos, 1.0, 0.01, u_time, audio);
  // color.g *= audio.lowpass * 2.0;
