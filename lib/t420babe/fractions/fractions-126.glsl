...
float fractions_126_map(vec3 p3, float time) {
...
void fractions_126(vec3 p3, float time, peakamp audio) {
  ...
  float rz = fractions_126_map(p3, time);

  float y = 1.0 * (sin(p3.y) + sin(p3.z * time));

  float m = plot(vec2(p3.x, p3.y * audio.notch), y * 10.0  * audio.notch, 5.126) * 1.0;

  float f =  ( rz - fractions_126_map(p3 * 1.0, wrap_time(time, 10.0)) ) ;
