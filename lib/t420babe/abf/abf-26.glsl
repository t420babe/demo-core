...
float abf_26_map(vec3 p3, float time) {
...
void abf_26(vec3 p3, float time, peakamp audio) {
  audio.lowpass   *= 1.0;
  audio.highpass  *= 1.0;
  audio.bandpass  *= 1.0;
  audio.notch     *= 1.0;
  ...
  float rz = abf_26_map(p3, time);
  float y = 1.0 * abs(sin(p3.x + 1.0) + sin(p3.x * time));
  float m = plot(vec2(p3.x, p3.y), y, 1.50) * 1.0;
  float r_mul = 0.1;
  float g_mul = 2.0;
  float b_mul = 1.5;
  // float f = clamp( ( rz - abf_26_map(p3 * 5.0, wrap_time(time, 10.0)) ) * 1.0, 0.05, 10.0 );
  float f =  ( rz - abf_26_map(p3 * 5.0, wrap_time(time, 10.0)) ) * 1.0;
