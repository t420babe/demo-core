...
vec3 aba_00_four_dots(vec2 pos, vec3 color, peakamp audio) {
...
vec3 aba_00_alternate(in vec2 pos, vec3 color, peakamp audio) {
  pos = pos.yx;
...
void aba_00(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.0;
  audio.highpass  *= 1.0;
  ...
  color = aba_00_alternate(pos, color, audio);
  color = 1.0 - color;
