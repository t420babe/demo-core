...
vec2 nye_2021_02_hsv(vec2 pos, float u_time) {
  float a = 1.0;
  float u_0 = 10.0;
...
float nye_2021_02_vc_nyc(vec2 pos, float u_time, peakamp audio) {
  pos = pos.yy;
  pos.x += 0.0;
  vec2 uv_int = nye_2021_02_hsv(pos, u_time);
...
float nye_2021_02_shape(vec2 pos, float radius, float u_time, peakamp audio) {
  ...
  f1 = nye_2021_02_vc_nyc(pos, u_time, audio);

  // return 1.0 - smoothstep(f1, f + 1.007, r) / fwidth(f1);
  return 1.0 - sharp(smoothstep(f1, f + 1.007, r) );
}

float nye_2021_02_shape_border(vec2 pos, float radius, float width, float u_time, peakamp audio) {
  return nye_2021_02_shape(pos, radius, u_time, audio) - nye_2021_02_shape(pos, radius - width, u_time, audio);
}


vec3 nye_2021_02(vec2 pos, float u_time, peakamp audio) {
  vec2 st = pos;
  vec3 color = vec3(1.0);
  audio.lowpass   *= 2.0;
  audio.highpass  *= 2.0;
  ...
  // nye_2021_02_shape_color_border(pos, 1.0, 0.10, u_time, audio, color);

  // Color 0
  // color.b += audio.lowpass * 2.0;
  // color *= nye_2021_02_shape_border(pos, 1.0, 0.50, u_time, audio);
  ...
  color /= nye_2021_02_shape_border(pos * 0.8, 1.0, 5.10, u_time, audio);
