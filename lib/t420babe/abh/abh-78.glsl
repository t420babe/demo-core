...
#endif
mat2 abh_78_blob_m(float a){
  float c=tan(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float abh_78_blob(vec3 p3, float time){
    p3.xz *= abh_78_blob_m(time * 0.4);
    p3.xy*= abh_78_blob_m(time * 0.3);
    vec3 q = p3 * 2.0 + time;
    float x0 = length( p3 );
    float x1 = sin( length(p3) + 1.0 );
    float x2 = atan( q.y + atan( q.z + sin(q.y) ) ) * 0.5 - 0.0;
    return x0 * x1 + x2;
}


void abh_78(vec3 p3, float time, peakamp audio) {
  ...
  float rz = abh_78_blob(p3, time);
  float f = ( rz * abh_78_blob(p3, y) ) * 1.0;
