...
vec3 aba_04_alternate(in vec2 pos, vec3 color, float time, peakamp audio) {
...
void aba_04(vec3 p3, float time, peakamp audio) {
  vec3 color = vec3(1.0);
  vec2 pos = p3.xy;
  float mul = 1.1;
  audio.lowpass   *= mul;
  ...
  color = aba_04_alternate(pos, color, time, audio);
  color = 1.0 - color;


  gl_FragColor = vec4(color * 0.811, 1.0);
}
#endif
