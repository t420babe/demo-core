// #chill 
// Idontknow- 7" Version by Jamie xx
#ifndef T4B_ABH_38
#define T4B_ABH_38
...
#endif
mat2 abh_38_rotate2d(float a){
  float c=cos(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float abh_38_blob(vec3 p3, float time){
  p3 = p3.yxz;
...
void abh_38(vec3 p3, float time, peakamp audio) {
  // audio.lowpass = 0.05;
  // audio.highpass = 0.05;
  // audio.bandpass = 0.05;
  // audio.notch = 0.05;
  ...
  vec3 color = vec3(1.0);
  p3.y *= 0.75;
  p3.xz *= rotate2d(time * 0.25);
  p3.xy *= rotate2d(time * 0.25);
  p3 *= 10.0;
  float y = (tan(p3.x) * cos(p3.x * time * 3.0));
  float m = plot(vec2(p3), y, 0.25) * 1.0;

  // p3.x -= 0.5;
  float rz = abh_38_blob(1.0 * p3, time);
  float f = ( rz / abh_38_blob(p3, y) ) * 10.0;
