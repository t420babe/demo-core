...
vec3 r6_mod289(vec3 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
vec2 r6_mod289(vec2 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
vec3 r6_permute(vec3 x) { return r6_mod289(((x*34.0)+1.0)*x); }

float r6_snoise(vec2 v) {
  ...
  // Do some r6_permutations to avoid
  // truncation effects in r6_permutation
  i = r6_mod289(i);
  vec3 p = r6_permute(
      r6_permute( i.y + vec3(0.0, i1.y, 1.0))
...
float r6_ridge(float h, float offset) {
...
float r6_ridgedMF(vec2 p, float u_t) {
  ...
  for(int i=0; i < OCTAVES; i++) {
    // float n = r6_ridge(r6_snoise(p*freq * tan( 0.05 * u_t + sin(u_t))), offset);
    // float n = r6_ridge(r6_snoise(p*freq * tan( 1.05 *  sin(u_t))), offset);
    float n = r6_ridge(r6_snoise(p*freq * fract( 1.05 *  atan(0.5 * u_t))), offset + move_time);
    sum += n*amp;
...
void r6_ridge_main(vec2 pos, float u_time, peakamp audio, out vec3 color) {
  ...
  if (audio.bandpass > 1.0) {
    color += r6_ridgedMF(pos *6.0, audio.bandpass); 
  }
