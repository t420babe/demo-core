#ifndef T420BABE_CHOICE_48
#define T420BABE_CHOICE_48
...
float choice_48_map(vec3 pos, float time){
...
vec3 choice_48(vec2 pos, float time, peakamp audio) {
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.5;
  audio.highpass  *= 1.5;
  audio.bandpass  *= 1.5;
  ...
  for(int i = 0; i <= 10; i++)	{
    ...
    pos *= atan(time * 0.5) * cos(time * 0.1) + 0.5;
    float rz = choice_48_map(pos, time);
    float f = clamp( ( rz - choice_48_map(pos + 0.5, time) ) * 0.5, 0.1, 1.0 );
