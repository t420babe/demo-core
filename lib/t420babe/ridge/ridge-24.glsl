...
vec3 r24_permute(vec3 x) { return r24_mod289(((x*0.05)+1.0)*x); }
float r24_snoise(vec2 v) {
  ...
  i = r24_mod289(i);
  vec3 p = r24_permute(
      r24_permute( i.y + vec3(0.0, i1.y, 1.0))
...
float r24_ridge(float h, float offset) {
...
float r24_ridgedMF(vec2 p, float u_time) {
  ...
  for(int i=0; i < octaves; i++) {
    // float n = r24_ridge(r24_snoise(p*freq * tan( 0.05 * u_time + sin(u_time))), offset);
    // float n = r24_ridge(r24_snoise(p*freq * tan( 1.05 *  sin(u_time))), offset);
    float n = r24_ridge(r24_snoise(p*freq * fract( 1.05 *  atan(0.5 * u_time))), offset + move_time);
    sum += n*amp;
...
void r24_ridge_main(vec2 pos, float u_time, peakamp audio, out vec3 color) {
  ...
  float zoom = 0.05;
  color += r24_ridgedMF(pos*5.0, audio.lowpass * pos.y * zoom); 
