...
#ifndef T420BABE_CHOICE_15
#define T420BABE_CHOICE_15
...
float choice_15_map(vec3 pos, float time){
...
vec3 choice_15(vec2 pos, float time, peakamp audio) {
  // 365.0
  ...
  for(int i = 0; i <= 8; i++)	{
    ...
    float rz = choice_15_map(pos, time);
    float dim = 1.0;
    float f = clamp( ( rz - choice_15_map(pos + 0.5, wrap_time(time, 10.0)) ) * dim, 0.5, 5.0 );
    float r_mul = 1.1;
    float g_mul = 2.0;
    float b_mul = 1.5;
