...
#ifndef T4B_FRACTIONS_04
#define T4B_FRACTIONS_04

#ifndef COMMON_COMMON
#include "lib/common/00-common.glsl"
#endif

float f_04_plot(vec2 p2, float m) {
  return smoothstep(m - 0.02, m, p2.y) - smoothstep(m, m + 0.02, p2.y);
}

void fractions_04(vec3 p3, float time, peakamp audio) {
  vec3 color = vec3(1.0);
  ...
  float m = f_04_plot(vec2(p3.x, p3.y), y) * 10.0;
