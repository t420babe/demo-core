...
#endif
mat2 b2b_28_blob_m(float a){
  float c=cos(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float b2b_28_blob(vec3 p3, float time){
    p3.xz *= rotate2d(time * 0.4);
    p3.xy*= rotate2d(time * 0.3);
    vec3 q = p3 * 2.0;
    float x0 = length( p3 + vec3( sin(time * 0.7) ) );
    float x1 = log( length(p3) + 1.0 );
    float x2 = sin( q.x + sin( q.y + sin(q.y) ) ) * 0.5 - 1.0;
    return x0 * x1 + x2;
}


void b2b_28(vec3 p3, float time, peakamp audio) {
  // audio.lowpass   *= 2.0;
  // audio.highpass  *= 2.0;
  // audio.bandpass  *= 2.0;
  // audio.notch     *= 2.0;
  ...
  p3.x -= 0.5;
  float rz = b2b_28_blob(p3, time);
  float f = ( rz / b2b_28_blob(p3, y) ) * 10.0;
