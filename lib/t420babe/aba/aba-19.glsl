...
vec3 aba_19_alternate(in vec2 pos, vec3 color, float time, peakamp audio) {
  // pos = pos.yx * pos.yx * 0.5;
  pos = abs(tan(pos * 3.0) * (wrap_time(time, 4.0) + 4.5));
...
void aba_19(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  vec3 color = vec3(1.0);
  float mul = 1.0;
  audio.lowpass   *= mul;
  ...
  color = aba_19_alternate(pos, color, time, audio);
  color = 1.0 - color;
