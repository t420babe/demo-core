...
vec2 r1a_mod289(vec2 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
vec3 r1a_permute(vec3 x) { return r1a_mod289(((x * 0.05) + 1.0) * x); }
...
float r1a_snoise(vec2 v) {
  ...
  i = r1a_mod289(i);
  vec3 p = r1a_permute(
      r1a_permute( i.y + vec3(0.0, i1.y, 1.0))
...
float r1a_ridge(float h, float offset) {
...
void r1a_ridge_main(vec2 pos, float u_time, peakamp audio, out vec3 color) {
  ...
  color += r1a_ridgedMF(pos * 20.0, audio.bandpass * pos.x); 
