...
float fractions_86_map(vec3 p3, float time) {
...
void fractions_86(vec3 p3, float time, peakamp audio) {
  audio.lowpass *= 2.0;
  audio.highpass *= 2.0;
  audio.bandpass *= 2.0;
  audio.notch *= 2.0;
  ...
  float rz = fractions_86_map(p3, time);
  ...
  float f = clamp( ( rz - fractions_86_map(p3 + 0.5, wrap_time(time, 10.0)) ) * 1.0, 0.05, 10.0 );
