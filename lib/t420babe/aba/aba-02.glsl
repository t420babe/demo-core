...
vec3 aba_02_alternate(in vec2 pos, vec3 color, peakamp audio) {
  // pos = pos.yx;
...
void aba_02(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  pos *= 10.0;
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.0;
  ...
  color = aba_02_alternate(pos, color, audio).grb;

   gl_FragColor = vec4(color, 1.0);
   gl_FragColor -= texture2D(u_fb, vec2(tan(p3.x + 0.0), circle_1(p3.xy, p3.y)) + 0.5);
   gl_FragColor /= texture2D(u_fb, audio.lowpass * 0.1 * vec2(sin(p3.x + 0.0), tan(p3.y)) + 0.5);
}
#endif
