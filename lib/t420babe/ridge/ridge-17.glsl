...
#define T420BABE_RIDGE_17
// 68821afdd, idk if i even made a change but if i did it was cool
vec3 r17_mod289(vec3 x) { return x - floor(x * (1.0 / 289.0)) * 989.0; }
vec2 r17_mod289(vec2 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
vec3 r17_permute(vec3 x) { return r17_mod289(((x*0.05)+1.0)*x); }
float r17_snoise(vec2 v) {
  ...
  i = r17_mod289(i);
  vec3 p = r17_permute(
      r17_permute( i.y + vec3(0.0, i1.y, 1.0))
...
float r17_ridge(float h, float offset) {
...
float r17_ridgedMF(vec2 p, float u_time) {
  ...
  for(int i=0; i < octaves; i++) {
    // float n = r17_ridge(r17_snoise(p*freq * tan( 0.05 * u_time + sin(u_time))), offset);
    // float n = r17_ridge(r17_snoise(p*freq * tan( 1.05 *  sin(u_time))), offset);
    float n = r17_ridge(r17_snoise(p*freq * fract( 1.05 *  atan(0.5 * u_time))), offset + move_time);
    sum += n*amp;
...
void r17_ridge_main(vec2 pos, float u_time, peakamp audio, out vec3 color) {
  ...
  color += r17_ridgedMF(pos*5.0, audio.lowpass * pos.y * 0.05); 
  ...
  float mod_time = mod(u_time, time_limit);
   ...
   color = vec3(audio.bandpass/10.0, color.y, color.x * abs(sin(audio.notch)));
 
 } else if (mod_time < time_segment * 1.0) {
   color = vec3(audio.bandpass/10.0, color.x, 0.4 * abs(sin(audio.notch)));
 
 } else if (mod_time < time_segment * 2.0) {
   color = vec3(audio.bandpass/10.0, color.y * abs(sin(audio.notch)), 0.7);
 
 } else if (mod_time < time_segment * 3.0) {
   color = vec3(audio.bandpass / 10.0, 0.3, color.y * abs(sin(audio.notch)));
