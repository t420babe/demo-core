mat2 rot(float theta){
    return mat2(cos(theta), -sin(theta), sin(theta), cos(theta));
}
float map(vec3 p3, float time) {
  p3.xz *= rot(time * 0.3);
  p3.xy *= rot(time * 0.2);

  vec3 q = p3 * 2.0 + time;
  float x0 = length(p3 + vec3( (time * 0.7) ) );
  float x1 = sin(length(p3) + 1.0);
  float x2 = tan(p3.x * (q.z + (q.x / q.y) ) ) * 5.5;
  return x0 * x1 + x2 * 5.0;
}
void she_knew(vec3 p3, float time, peakamp audio) {
  time += 10.0;
  vec3 color = vec3(1.0);
  p3 *= time * cos(p3.y * log(p3.x)) * cos(p3.y)* 0.5;
  p3.xz *= rot(time * 1.40);
  p3.yx *= -rot(time * 0.8);
  float y1 = 1.0 * (sin(p3.x ));
  float m1 = plot(vec2(p3.x, p3.y), y1, 15.5);
  p3.xy *= m1;
  float rz = map(p3, time);
  float y = 1.0 * (sin(p3.y) + sin(p3.z * time));
  float m = plot(vec2(p3.x, p3.y * u_n), y * 10.0  * u_n, 5.50) * 1.0;
  float f =  ( rz - map(p3 * 1.0, wrap_time(time, 10.0)) ) ;
  vec3 l = vec3(u_n) * asin(0.1 * f * p3.y) + cos(0.1 * f * p3.y);
  color *= fract(l) * m * l;
  color.g -= 1.0 * u_hp;
  color.b *= sin(time) * 5.0;
  color.r /= pow(u_lp, 3.0);
  color.g *= cos(time) * 5.0;
  color = 1.0 - color;
  gl_FragColor = vec4(color, 1.0);
}
