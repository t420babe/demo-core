...
#ifndef T420BABE_CHOICE_05
#define T420BABE_CHOICE_05
...
float choice05_map(vec3 pos, float time){
...
vec3 choice_05(vec2 pos, float time, peakamp audio) {
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.0;
  audio.highpass  *= 1.0;
  audio.bandpass  *= 1.0;
  ...
  for(int i = 0; i <= 10; i++)	{
    ...
    float rz = choice05_map(pos, time);
    float f = clamp( ( rz - choice05_map(pos + 0.5, time) ) * 0.5, 0.1, 2.0 );
    float r_mul = 1.0;
    float g_mul = 2.0;
    float b_mul = 2.5;
