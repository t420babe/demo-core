...
float choice_13_map(vec3 pos, float time){
...
vec3 choice_13(vec2 pos, float time, peakamp audio) {
  // 365.0
  ...
  for(int i = 0; i <= 7; i++)	{
    ...
    float rz = choice_13_map(pos, time);
    float dim = 1.0;
    float f = clamp( ( rz - choice_13_map(pos + 0.5, wrap_time(time, 10.0)) ) * dim, 0.5, 5.0 );
    float r_mul = 0.1;
    float g_mul = 2.0;
    float b_mul = 1.5;
    ...
    vec3 l = vec3(0.35, 0.1, 0.3) + vec3(r_mul, g_mul, b_mul) * cos(f) * 4.0 / (0.3 * (pos.y - 30.2));
    color -= l;
    color += ( 1.0 - smoothstep(0.0, 0.1, rz ) ) * 1.0 * l * (abs(audio.notch) + 0.3);
