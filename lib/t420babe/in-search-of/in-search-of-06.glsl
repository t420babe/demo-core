...
vec2 iso_06_cellular2x2x2(vec3 P) {
...
float iso_06_spiral_pxl(vec2 st, float t) {
    ...
    return (((sin(r) * t  * log(a * sin(u_time)) / 1.0 * 1.000)));
}
...
vec3 iso_06(vec2 pos, float u_time, peakamp audio) {
  ...
  st *= 25.0 * abs(sin(u_time * 0.01));
	vec2 F = iso_06_cellular2x2x2(vec3(st * 1.0, u_time));
	float n = smoothstep(0.0, abs(sin(u_time * 0.05)) + 1.0, F.x) / ( abs(audio.notch * 0.5));
  // n = step(n, sin(pos.x));
  color = vec3(n);
  pos *= 2.0;
  // color -= iso_06_spiral_pxl(abs(sin(pos.yy) * cos(pos.xy)) * 3.0 * abs(audio.bandpass), 1.0 * wrap_time(u_time, 10.0) + 10.0);
  color -= iso_06_spiral_pxl(3.0 * pos.yx * abs(audio.bandpass), 1.0 * wrap_time(u_time, 10.0) + 00.0);
