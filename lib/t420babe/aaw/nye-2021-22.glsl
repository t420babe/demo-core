...
vec3 nye_2021_22_xtc_nyc(vec2 pos, float u_time, peakamp audio) {
  // pos /= 6.0;
...
vec2 nye_2021_22_hills_spherical_vortex(vec2 pos, float u_time) {
  float a = 1.0;
  float u_0 = 10.0;
...
float nye_2021_22_vc_nyc(vec2 pos, float u_time, peakamp audio) {
  pos = tan(fract(pos.yx) * cos(pos.xy));
  // pos  = pos.xx;
  pos.x += 5.0 * abs(tan(u_time * 0.5));
  pos.y -= 1.0 * abs(tan(u_time * 0.5));
  vec2 uv_int = nye_2021_22_hills_spherical_vortex(pos, u_time);
...
float nye_2021_22_shape(vec2 pos, float radius, float u_time, peakamp audio) {
  ...
  f1 = nye_2021_22_vc_nyc(pos, u_time, audio);

  // return 1.0 - smoothstep(f1, f + 1.007, r) / fwidth(f1);
  return 1.0 - sharp(smoothstep(f1, f + 1.007, r) );
}

float nye_2021_22_shape_border(vec2 pos, float radius, float width, float u_time, peakamp audio) {
  return nye_2021_22_shape(pos, radius, u_time, audio) - nye_2021_22_shape(pos, radius - width, u_time, audio);
}
float nye_2021_22_noise_clouds (in vec2 _st) {
    vec2 i = floor(_st);
    vec2 f = fract(_st);
...
float nye_2021_22_fbm ( in vec2 _st) {
    int onum = 5;
    float v = 0.0;
    float a = 0.5;
    ...
    for (int i = 0; i < onum; ++i) {
        v += a * nye_2021_22_noise_clouds(_st);
...
vec3 nye_2021_22_clouds(vec2 st, float u_time, peakamp audio) {
    st += st * abs(sin(u_time*0.1)*3.0);
    vec3 color = vec3(0.0);

    vec2 q = vec2(0.);
    q.x = nye_2021_22_fbm( st + 0.00*u_time);
    q.y = nye_2021_22_fbm( st + vec2(1.0));

    vec2 r = vec2(0.);
    r.x = nye_2021_22_fbm( st + 1.0*q + vec2(1.7,9.2)+ 0.15*u_time );
    r.y = nye_2021_22_fbm( st + 1.0*q + vec2(8.3,2.8)+ 0.126*u_time);

    float f = nye_2021_22_fbm(st+r);
...
float nye_2021_22_circle_sdf(vec2 pos, float radius) {
...
vec3 nye_2021_22(vec2 pos, float u_time, peakamp audio) {
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.0;
  audio.highpass  *= 1.0;
  audio.bandpass  *= 1.0;
  ...
  color = nye_2021_22_xtc_nyc(pos, u_time, audio);

  // // Color 0
  vec3 clouds_color = nye_2021_22_clouds(pos, u_time, audio);
