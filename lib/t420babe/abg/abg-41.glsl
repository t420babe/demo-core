...
void abg_41(vec3 p3, float time, peakamp audio) {
  // time += 200.0;
  vec2 pos = p3.xy;
  ...
  // return hsv_color;
  gl_FragColor = vec4(0.8 - color.gbr, 1.0);
  ...
  float effect = time;
  gl_FragColor -= texture2D(u_fb, vec2(g_pos.yx/effect + .5) + vec2(0.001, 0.00)) + 0.001;
}
#endif
