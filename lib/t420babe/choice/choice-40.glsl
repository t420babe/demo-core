#ifndef T420BABE_CHOICE_39
#define T420BABE_CHOICE_39
...
float choice_39_map(vec3 pos, float time, peakamp audio){
...
vec3 choice_39(vec2 pos, float time, peakamp audio) {
  // 365.0
  ...
  for(int i = 0; i <= 3; i++)	{
    ...
    float rz = choice_39_map(pos3, time, audio);
    float dim = 1.0;
    // float f = clamp( ( rz - choice_39_map(pos * audio.notch * 3.0, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );

    // float f = clamp( ( rz - choice_39_map(pos + 0.5, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );
    float f = clamp( ( rz - choice_39_map(abs(sin(pos3 * 5.0)) * audio.lowpass * 4.0, wrap_time(time, 10.0), audio) ) * dim, 0.1, 5.0 );
