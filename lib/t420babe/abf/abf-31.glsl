...
float abf_31_map(vec3 p3, float time) {
...
void abf_31(vec3 p3, float time, peakamp audio) {
  audio.lowpass   *= 1.0;
  audio.highpass  *= 1.0;
  audio.bandpass  *= 1.0;
  audio.notch     *= 1.0;
  ...
  float rz = abf_31_map(p3, time);
  float y = 1.0 * (sin(p3.y + 1.0) + sin(p3.z * time));
  float m = plot(vec2(p3.x, p3.z), y, 5.50) * 1.0;
  // float f = clamp( ( rz - abf_31_map(p3 * 5.0, wrap_time(time, 10.0)) ) * 1.0, 0.05, 10.0 );
  float f =  ( rz - abf_31_map(p3 * 1.0, wrap_time(time, 10.0)) ) ;
