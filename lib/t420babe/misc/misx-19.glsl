...
vec3 misx_19(vec2 pos, float u_time, peakamp audio) {
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.0;
  audio.highpass  *= 1.0;
  audio.bandpass  *= 1.0;
  ...
  color = vec3(0.5457, 0.034, 0.537) - color;
  // color = color.grb;
  color = color.rgb;

  color.b *= abs(audio.notch) * 1.8;
  color.r *= abs(audio.notch) * 1.8;
  color.g *= abs(audio.notch) * 1.8;
  // color = 1.0 - color;
  return color;
}
