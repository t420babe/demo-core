...
float couch189_noise (in vec2 st, peakamp audio) {
    // vec2 i = log(st.xx *  0.00001);
    vec2 i = (tan(st.xx *  0.0001));
    vec2 f = fract((st.yy * 0.0001));

    // Four corners in 2D of a tile
    float a = couch189_random(i);
    float b = couch189_random(i * f + vec2(1.0, 10.0 * audio.bandpass));
    float c = couch189_random(i * f + vec2(0.0, 1.0));
    float d = couch189_random(i * f + vec2(1.0, 1.0));
...
float couch189_fbm (in vec2 st, peakamp audio) {
    st.y += 2.5;
    // st /= 10.0;
    // Initial values
    ...
    for (int i = 0; i < octaves; i++) {
        value += amplitude * couch189_noise(st, audio);
        st *= 18.0;
