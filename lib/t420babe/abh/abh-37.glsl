...
#endif
mat2 bg_37_rotate2d(float a){
  float c=cos(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float bg_37_blob(vec3 p3, float time){
...
void bg_37(vec3 p3, float time, peakamp audio) {
  // audio.lowpass = 0.05;
  // audio.highpass = 0.05;
  // audio.bandpass = 0.05;
  // audio.notch = 0.05;
  ...
  // p3.x -= 0.5;
  float rz = bg_37_blob(1.0 * p3, time);
  float f = ( rz / bg_37_blob(p3, y) ) * 10.0;
