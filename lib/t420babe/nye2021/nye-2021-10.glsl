...
vec2 nye_2021_10_hills_spherical_vortex(vec2 pos, float u_time) {
  float a = 1.0;
  float u_0 = 10.0;
...
float nye_2021_10_vc_nyc(vec2 pos, float u_time, peakamp audio) {
  pos = pos.yy;
  pos.x += 5.0 * (sin(u_time * 0.5));
  vec2 uv_int = nye_2021_10_hills_spherical_vortex(pos, u_time);
...
float nye_2021_10_shape(vec2 pos, float radius, float u_time, peakamp audio) {
  ...
  f1 = nye_2021_10_vc_nyc(pos, u_time, audio);

  // return 1.0 - smoothstep(f1, f + 1.007, r) / fwidth(f1);
  return 1.0 - sharp(smoothstep(f1, f + 1.007, r) );
}

float nye_2021_10_shape_border(vec2 pos, float radius, float width, float u_time, peakamp audio) {
  return nye_2021_10_shape(pos, radius, u_time, audio) - nye_2021_10_shape(pos, radius - width, u_time, audio);
}
...
vec3 nye_2021_10(vec2 pos, float u_time, peakamp audio) {
  ...
  color *= nye_2021_10_shape_border(pos, 3.0, 1.00, u_time, audio);
  ...
  // color /= nye_2021_10_shape_border(pos * 0.8, 1.0, 5.10, u_time, audio);
