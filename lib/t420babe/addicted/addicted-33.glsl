...
vec3 addicted_33_29_12(vec2 pos, float time, peakamp audio, vec2 res) {
  vec2 uv = pos.yx;
  uv *= 15.0;
  vec3 color = vec3(1.0);
...
vec3 addicted_33_29_02(vec2 pos, float time, peakamp audio, vec2 res) {
  vec2 uv = pos;
  vec3 color = vec3(1.0);
...
vec3 addicted_33_29(vec2 pos, float time, peakamp audio, vec2 res) {
  vec3 color = vec3(1.0);
...
vec3 addicted_33(vec2 pos, float time, peakamp audio, vec2 res) {
  vec3 color = vec3(1.0);

  float t = wrap_time(time * 0.1, 1.0);
  vec3 color_0 = addicted_29(pos, u_at * 1.0, audio, u_resolution);
  pos = rotate2d(sin(u_time * 0.1) * 3.14 / 5.0) * pos;
  pos.x = 1.0 * cos(pos.x);
  pos.y = 1.0 * sin(pos.y);
  vec3 color_1 = addicted_31(pos, u_at * 1.0, audio, u_resolution);
  color = mix(color_1, color_0, t);
  color = addicted_32(pos, u_at * 1.0, audio, u_resolution);
