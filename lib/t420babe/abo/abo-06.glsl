...
vec4 map(vec3 p, float time, peakamp audio) {
  ...
  // p.xz *= rotate2d(sin(time*.06)*20.5);
...
void abo_06(vec3 p3, float time, peakamp audio, vec4 frag_coord, vec2 u_res) {
  vec3 col = vec3(1.0);
