...
vec3 nye_2021_19(vec2 pos, float u_time, peakamp audio) {
  ...
  color *= sin(circle_sdf(pos, abs(0.1 * tan(u_time * 0.5)) * abs(audio.notch)));
  // color *= sin(circle_sdf(pos, abs(1.0 * sin(u_time * 0.5)) * abs(audio.notch)));
  color = 1.0 - color;
  ...
  color.r *= abs(audio.lowpass * 3.5);
  // color.g -= abs(audio.lowpass * 1.0);

  color -= clouds_color / vec3(0.5, 0.23423, 0.2540);
  color = color.bgr;
  // color = color.grb;
  //
  //
