...
#define T420BABE_RIDGE_14
// f38b535, 01:53
vec3 r14_mod289(vec3 x) { return x - floor(x * (1.0 / 289.0)) * 989.0; }
vec2 r14_mod289(vec2 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
vec3 r14_permute(vec3 x) { return r14_mod289(((x*0.05)+1.0)*x); }
float r14_snoise(vec2 v) {
  ...
  i = r14_mod289(i);
  vec3 p = r14_permute(
      r14_permute( i.y + vec3(0.0, i1.y, 1.0))
...
float r14_ridge(float h, float offset) {
...
float r14_ridgedMF(vec2 p, float u_time) {
  ...
  for(int i=0; i < octaves; i++) {
    // float n = r14_ridge(r14_snoise(p*freq * tan( 0.05 * u_time + sin(u_time))), offset);
    // float n = r14_ridge(r14_snoise(p*freq * tan( 1.05 *  sin(u_time))), offset);
    float n = r14_ridge(r14_snoise(p*freq * fract( 1.05 *  atan(0.5 * u_time))), offset + move_time);
    sum += n*amp;
...
void r14_ridge_main(vec2 pos, float u_time, peakamp audio, out vec3 color) {
  ...
  color += r14_ridgedMF(pos*32.0, audio.bandpass * pos.x); 
