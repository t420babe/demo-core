...
float r8_snoise(vec2 v) {
  ...
  i = r8_mod289(i);
  vec3 p = r8_permute(
      r8_permute( i.y + vec3(0.0, i1.y, 1.0))
...
float r8_ridge(float h, float offset, float audio_notch) {
...
float r8_ridgedMF(vec2 p, float u_time, float audio_notch) {
  ...
  for(int i=0; i < octaves; i++) {
    // float n = r8_ridge(r8_snoise(p*freq * tan( 0.05 * u_time + sin(u_time))), offset);
    // float n = r8_ridge(r8_snoise(p*freq * tan( 1.05 *  sin(u_time))), offset);
    float n = r8_ridge(r8_snoise(p*freq * fract( 1.05 *  atan(0.5 * u_time))), offset + move_time, audio_notch);
    sum += n*amp;
...
void r8_ridge_main(vec2 pos, float u_time, peakamp audio, out vec3 color) {
  ...
  // pos -= 0.1;
  // st += audio.notch;
  color += r8_ridgedMF(pos*25.0, audio.bandpass * pos.y * 50.0, audio.notch); 
