...
#endif
mat2 bg_29_blob_m(float a){
  float c=cos(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float bg_29_blob(vec3 p3, float time){
...
void bg_29(vec3 p3, float time, peakamp audio) {
  // audio.lowpass   *= 2.0;
  // audio.highpass  *= 2.0;
  // audio.bandpass  *= 2.0;
  // audio.notch     *= 2.0;
  ...
  p3.x -= 0.5;
  float rz = bg_29_blob(p3, time);
  float f = ( rz / bg_29_blob(p3, y) ) * 10.0;
