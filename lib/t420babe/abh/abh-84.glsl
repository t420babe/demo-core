...
void abh_84(vec3 p3, float time, peakamp audio) {
  vec3 color = vec3(1.0);
  p3 *= 7.0;
  ...
  float rz = blob(2.0 * fract(sin(p3)), time);
  float f = ( rz - blob(p3, y) ) * 1.0;
  // color = (m) * color * m * vec3(1.0);
  vec3 l = vec3(0.25, 0.8, 0.7) + vec3(abs(audio.lowpass), abs(audio.bandpass), abs(audio.notch)) * f;
  color = l;
