...
float choice_45_map(vec3 pos, float time){
...
vec3 choice_45(vec2 pos, float time, peakamp audio) {
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.0;
  audio.highpass  *= 1.0;
  audio.bandpass  *= 1.0;
  ...
  for(int i = 0; i <= 10; i++)	{
    ...
    float rz = choice_45_map(pos, time);
    float f = clamp( ( rz - choice_45_map(pos + 0.5, time) ) * 0.5, 0.1, 1.0 );
