// #katie1
#ifndef T4B_FRACTIONS_134
#define T4B_FRACTIONS_134
...
float f_134_plot(vec2 p2, float m) {
  return smoothstep(m - 0.15, m, p2.y) - smoothstep(m, m + 0.15, p2.y);
}

void fractions_134(vec3 p3, float time, peakamp audio) {
  // time += 100.0;
  vec3 color = vec3(1.0);
  // p3 *= time * 0.5;
  // p3 *= 3.0;
  ...
  // p3.x *= 1.1;
  p3 *= 0.20;
  float y1 = audio.highpass * (tan(p3.x * 10.0) + cos(p3.x * time));
  ...
  // float m = f_134_plot(vec2(p3.x, p3.y), y) * 1.0;
  ...
  // gl_FragColor = vec4(rgb2hsv(color.grb), 1.0);
  gl_FragColor = vec4(rgb2hsv(1.0 - color.rbg), 1.0);
  // gl_FragColor += texture2D(u_fb, vec2(p3.x + 0.0, p3.y + 0.5));
  gl_FragColor += texture2D(u_fb, sin(vec2(p3.yx/2.+.5) + vec2(0.001, 0.00)) - 0.002);
