...
mat2 abp_07_scale(vec2 _abp_07_scale) {
  return mat2(_abp_07_scale.x, 0.0, 0.0, _abp_07_scale.y);
}

// Overload for scalar 
mat2 abp_07_scale(float _abp_07_scale) {
  return mat2(_abp_07_scale, 0.0, 0.0, _abp_07_scale);
}

float spinning_cirlces_illusion(vec2 st, float time, float time_abp_07_scale, float offset, float smoothing_factor){
 ...
 // Translate coords - remember origin in in the btm left
  ...
  for(int i = 0; i < 20; i++){
    ...
    // st = abp_07_scale(1.5)*st;
    float new_center_x = r * cos(float(i) * TWO_PI / 18.);
    float new_center_y = r * sin(float(i) * TWO_PI );
 // Relative to translated coord sys (dividing with numbers with various common factors leads to more symmetric or chaotic ring resonance)
    ...
    // Translate coords - remember origin in in the btm left
    st -= center;
    st = rotate2d(time * time_abp_07_scale) * st;
    // circle += plot(st, r);
...
void abp_07(vec3 p3, float time, peakamp audio) {
  vec2 st = p3.xy;
  st += 0.5;
  vec2 xy = st;
  ...
  // color += circle;
  color += spinning_cirlces_illusion(st, time, 0.15, 1.0, 0.005);
