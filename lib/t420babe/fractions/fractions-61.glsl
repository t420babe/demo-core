...
#ifndef T4B_FRACTIONS_61
#define T4B_FRACTIONS_61
...
float fractions_61_map(vec3 p3, float time) {
...
void fractions_61(vec3 p3, float time, peakamp audio) {
  ...
  float rz = fractions_61_map(p3, time);
  ...
  float f =  ( rz / fractions_61_map(p3, wrap_time(time, 10.0)) ) ;
  ...
  color.r *= 15.0 * audio.notch;
  color.b -= 10.0 * audio.bandpass;
  color.g /= 10.0 * audio.lowpass;
