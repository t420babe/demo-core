...
vec3 addicted_20_23(vec2 pos, float time, peakamp audio, vec2 res) {
  vec2 uv = pos;
  vec3 color = vec3(1.0);
...
vec3 addicted_20_26(vec2 pos, float time, peakamp audio, vec2 res) {
  vec2 uv = pos;
  vec3 color = vec3(1.0);
...
// u_at = 1300
vec3 addicted_20(vec2 pos, float time, peakamp audio, vec2 res) {
  vec3 color = vec3(1.0);

  float t = 0.4;

  vec3 color_0 = addicted_20_26(pos.yx, u_at * 1.0, audio, u_resolution);
  vec3 color_1 = addicted_20_23(pos, u_at * 1.0, audio, u_resolution);
