...
float aaz_19_spiral(vec2 st, float t) {
...
void aaz_19(vec3 p3, float time, peakamp audio) {
  ...
  pos *= 3.0;
  // pos.y += 1.5;
  color -= abs(sin(time * 0.1) + 2.0) / aaz_19_spiral(abs(sin(pos.yx) / atan(pos.xy)) * 4.5 * abs(1.0 * audio.bandpass), 0.1 * wrap_time(time, 10.0) + 10.0);
  color -= aaz_19_spiral(3.0 * pos.yx * abs(audio.bandpass), 1.0 * wrap_time(time, 10.0) + 10.0);
