...
float contour_lines(vec2 pos, float u_time, peakamp audio, inout vec3 color) {
    float n = simplex3( vec3( pos * scale, u_time * .02)) * 35.;
    n = smoothstep( 1.5, 0., abs(fract(n+.5)-.5) / fwidth(n) ); 
		return n;
}
