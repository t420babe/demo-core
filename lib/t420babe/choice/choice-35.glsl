#ifndef T420BABE_CHOICE_35
#define T420BABE_CHOICE_35
...
float choice_35_map(vec3 pos, float time, peakamp audio){
  pos.xz *= rotate2d(time * 0.3 * cos(pos.x * 0.05)) * audio.notch * 0.9;
  pos.xy *= rotate2d(time * 0.3 * sin(pos.y * 0.05)) * audio.notch * 0.9;
...
vec3 choice_35(vec2 pos, float time, peakamp audio) {
  // 365.0
  ...
  for(int i = 0; i <= 2; i++)	{
    ...
    float rz = choice_35_map(pos, time, audio);
    float dim = 1.0;
    // float f = clamp( ( rz - choice_35_map(pos * audio.notch * 3.0, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );

    // float f = clamp( ( rz - choice_35_map(pos + 0.5, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );
    float f = clamp( ( rz - choice_35_map(abs(sin(pos)) * audio.lowpass * 4.0, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );
