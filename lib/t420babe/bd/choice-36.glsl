#ifndef T420BABE_CHOICE_36
#define T420BABE_CHOICE_36
...
float choice_36_map(vec3 pos, float time, peakamp audio){
  pos.xz *= rotate2d(time * 0.3 * cos(pos.x * 0.1)) * audio.notch * 0.5;
  pos.yz *= rotate2d(time * 0.3 * sin(pos.y * 0.1)) * audio.notch * 0.5;
...
vec3 choice_36(vec2 pos, float time, peakamp audio) {
  // 365.0
  ...
  for(int i = 0; i <= 2; i++)	{
    ...
    float rz = choice_36_map(pos3, time, audio);
    float dim = 1.0;
    // float f = clamp( ( rz - choice_36_map(pos * audio.notch * 3.0, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );

    // float f = clamp( ( rz - choice_36_map(pos + 0.5, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );
    float f = clamp( ( rz - choice_36_map(abs(sin(pos3 * 1.0)) * audio.lowpass * 4.0, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );
