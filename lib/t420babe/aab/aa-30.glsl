...
// sense of place
vec3 aa_30_17(vec2 pos, float time, peakamp audio, vec2 res) {
...
vec3 aa_30(vec2 pos, float time, peakamp audio, vec2 res) {
  vec3 color = vec3(1.0);

  // @1:42:55, u_at = 420s
  float t = wrap_time(time * 0.1, 1.0);

  vec3 color_0 = aa_17(pos.yx, u_at * 2.0, audio, u_resolution);
  pos = rotate2d(sin(u_time * 0.1) * 3.14 / 1.0) * pos;
  vec3 color_1 = aa_17(pos, u_at * 2.0, audio, u_resolution);
