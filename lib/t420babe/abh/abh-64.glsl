...
float abh_64_blob(vec3 p3, float time){
  p3 = p3.yxz * 0.8;
...
void abh_64(vec3 p3, float time, peakamp audio) {
  // audio.lowpass = 0.05;
  // audio.highpass = 0.05;
  // audio.bandpass = 0.05;
  // audio.notch = 0.05;
  ...
  // p3.x -= 0.5;
  float rz = abh_64_blob(p3, time);
  float rz_2 = abh_64_blob(p3 + vec3(0.5), time);
  float f = ( rz * abh_64_blob(p3, y) ) * 1.0;
