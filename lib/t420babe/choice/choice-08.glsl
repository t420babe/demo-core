...
#ifndef T420BABE_CHOICE_08
#define T420BABE_CHOICE_08
...
float choice08_map(vec3 pos, float time){
...
vec3 choice_08(vec2 pos, float time, peakamp audio) {
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.2;
  audio.highpass  *= 1.2;
  audio.bandpass  *= 1.2;
  ...
  for(int i = 0; i <= 10; i++)	{
    ...
    float rz = choice08_map(pos, time);
    float f = clamp( ( rz - choice08_map(pos + 0.5, time) ) * 0.5, 0.1, 2.0 );
    float r_mul = 0.1;
    float g_mul = 2.0;
    float b_mul = 1.5;
