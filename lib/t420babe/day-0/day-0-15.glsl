...
#ifndef T420BABE_DAY_0_15
#define T420BABE_DAY_0_15
...
vec3 day_0_15(vec2 pos, float u_time, peakamp audio) {
  vec3 color = vec3(1.0);
  ...
  vec2 pos_l = pos_c;
  // pos_l.x += 0.5;
  float pct_l = sharp(circle_1(pos_l * 0.9, audio.notch)) * 2.0;
  float pct_c = sharp(circle_1(pos_c * 0.3, audio.notch)) * 2.0;
  color = vec3(pct_c * color - pct_l + color.bgr);
