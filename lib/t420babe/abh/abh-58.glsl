...
#endif
mat2 abh_03_rotate2d(float a){
  float c=cos(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float abh_03_blob(vec3 p3, float time){
  p3 = p3.yxz;
  // p3.xz *= rotate2d(time * 0.4);
  p3.xy *= rotate2d(time * 0.3);
  vec3 q = p3 * 2.0 + time;
  float x0 = length( (p3) + vec3( wrap_time(time * 0.7, 10.0) ) );
...
void abh_03(vec3 p3, float time, peakamp audio) {
  time = wrap_time(time * 0.5, 10.0);
  vec3 color = vec3(1.0);
  // p3.y *= 0.30;
  p3.xz *= rotate2d(time * 0.5);
  p3.xy *= rotate2d(time * 0.05);
  p3 *= 70.0;
  float y = (atan(p3.x) * cos(p3.x * time * 1.0));
  // float y = abs(sin(time) - cos(time / p3.x));

  float m = plot(vec2(p3), y, 0.25) * 1.0;

  // p3.x -= 0.5;
  float rz = abh_03_blob(p3, time);
  float f = ( rz * abh_03_blob(p3, y) ) * 1.0;
  ...
  // gl_FragColor += texture2D(u_fb, vec2(rz* p3.yx/2.+.5) + vec2(0.001, 0.00)) - 0.003;
  // gl_FragColor += texture2D(u_fb, vec2(p3.yx/2.+.5) + vec2(0.001, 0.00)) - 0.003;
