...
#define T420BABE_RIDGE_13
// f4eca7f, 01:46, only difference is taht we dont divie pos by 53 here
vec3 r13_mod289(vec3 x) { return x - floor(x * (1.0 / 289.0)) * 989.0; }
vec2 r13_mod289(vec2 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
vec3 r13_permute(vec3 x) { return r13_mod289(((x*0.05)+1.0)*x); }
float r13_snoise(vec2 v) {
  ...
  i = r13_mod289(i);
  vec3 p = r13_permute(
      r13_permute( i.y + vec3(0.0, i1.y, 1.0))
...
float r13_ridge(float h, float offset) {
...
float r13_ridgedMF(vec2 p, float u_time) {
  ...
  for(int i=0; i < octaves; i++) {
    // float n = r13_ridge(r13_snoise(p*freq * tan( 0.05 * u_time + sin(u_time))), offset);
    // float n = r13_ridge(r13_snoise(p*freq * tan( 1.05 *  sin(u_time))), offset);
    float n = r13_ridge(r13_snoise(p*freq * fract( 1.05 *  atan(0.5 * u_time))), offset + move_time);
    sum += n*amp;
...
void r13_ridge_main(vec2 pos, float u_time, peakamp audio, out vec3 color) {
  ...
  color += r13_ridgedMF(pos*11.0, audio.bandpass * pos.x); 
