...
float choice_44_map(vec3 pos, float time, peakamp audio){
...
vec3 choice_44(vec2 pos, float time, peakamp audio) {
  // 365.0
  ...
  for(int i = 0; i <= 2; i++)	{
    ...
    pos *= 50.0;
    float rz = choice_44_map(pos, time, audio);
    float dim = 1.0;

    float f = clamp( ( rz - choice_44_map(abs(sin(pos * time * 0.5)) * audio.lowpass * 4.0, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );
