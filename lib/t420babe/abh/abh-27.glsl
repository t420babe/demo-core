...
#endif
mat2 abh_27_blob_m(float a){
  float c=cos(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float abh_27_blob(vec3 p3, float time){
    p3.xz *= rotate2d(time * 1.4);
    p3.xy*= rotate2d(time * 1.3);
    vec3 q = p3 * 3.0 + time;
    float x0 = length( p3 + vec3( sin(time * 1.7) ) );
    float x1 = log( length(p3) + time * 0.001);
    float x2 = sin( q.x + sin( q.z + sin(q.y) ) ) * 0.5 - 1.0;
    return x0 * x1 + x2;
}


void abh_27(vec3 p3, float time, peakamp audio) {
  // audio.lowpass = 0.05;
  // audio.highpass = 0.05;
  // audio.bandpass = 0.05;
  // audio.notch = 0.05;
  ...
  p3.x -= 0.5;
  float rz = abh_27_blob(1.0 * p3, time);
  float f = ( rz / abh_27_blob(p3, y) ) * 10.0;
  // color = (m) * color * m * vec3(1.0);
  vec3 l = vec3(audio.lowpass, 0.3, 0.5) + vec3(abs(audio.lowpass), abs(audio.bandpass), abs(audio.notch)) * f;
  color = l;
