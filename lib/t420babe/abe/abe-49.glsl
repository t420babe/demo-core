...
//
// float abe_49_map(vec3 pos, float time){
...
vec3 abe_49_single(vec2 pos, float time, peakamp audio) {
  vec3 color = vec3(1.0);
  ...
  for(int i = 0; i <= 10; i++)	{
    ...
    float rz = abe_49_map(pos, time);
    float f = clamp( ( rz - abe_49_map(pos + 0.5, time) ) * 0.5, 0.1, 1.0 );
...
vec3 abe_49_multi(vec2 pos, float time, peakamp audio) {
  vec3 color = vec3(1.0);
...
void abe_49(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.3;
  audio.highpass  *= 1.3;
  ...
  if (time < t2s(0.0, 1.0, 35.0)) {
    color = abe_49_single(pos, time, audio);
  } else if (time > t2s(0.0, 1.0, 35.0) && time < t2s(0.0, 7.0, 30.0))  {
    color = abe_49_multi(pos, time, audio);
  } else {
    vec3 color_0 = abe_49_single(pos, time, audio);
    vec3 color_1 = abe_49_multi(pos, time, audio);
