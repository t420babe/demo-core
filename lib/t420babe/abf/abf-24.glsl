...
float abf_24_map(vec3 p3, float time) {
...
void abf_24(vec3 p3, float time, peakamp audio) {
  audio.lowpass *= 2.0;
  audio.highpass *= 2.0;
  audio.bandpass *= 2.0;
  audio.notch *= 2.0;
  ...
  float rz = abf_24_map(p3, time);
  float y = 1.0 * abs(sin(p3.x + 1.0) + sin(p3.x * time));
  float m = plot(vec2(p3.x, p3.y), y, 1.50) * 1.0;
  float r_mul = 0.1;
  float g_mul = 2.0;
  float b_mul = 1.5;
  float f = clamp( ( rz - abf_24_map(p3 + 0.5, wrap_time(time, 10.0)) ) * 1.0, 0.05, 10.0 );
