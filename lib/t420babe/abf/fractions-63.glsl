...
#ifndef T4B_FRACTIONS_63
#define T4B_FRACTIONS_63
...
float fractions_63_map(vec3 p3, float time) {
...
void fractions_63(vec3 p3, float time, peakamp audio) {
  ...
  float rz = fractions_63_map(p3, time);
  ...
  float f =  ( rz - fractions_63_map(p3 * 1.0, wrap_time(time, 10.0)) ) ;
  ...
  color.g /= 25.0 * audio.bandpass;
  color.b *= 5.0 * audio.notch;
  //
  // color.r += 10.0;
  // color.g *= 10.0;
  // color.b += 0.5;
