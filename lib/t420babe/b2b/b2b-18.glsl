...
#endif
mat2 b2b_18_blob_m(float a){
  float c=cos(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float b2b_18_blob(vec3 p3, float time){
...
void b2b_18(vec3 p3, float time, peakamp audio) {
  time += 100.0;
  vec3 color = vec3(1.0);
  p3 *= 1.5;
  ...
  p3.x -= 0.5;
  float rz = b2b_18_blob(1.0 * p3, time);
  float f = ( rz / b2b_18_blob(p3, y) ) * 10.0;
