...
vec3 l15_alternate(in vec2 pos, vec3 color, float time, peakamp audio) {
  ...
  float mul = (clamp(sin(time * 0.5), 0.5, 1.0) + 0.05) * 0.5;
...
void aba_15(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  vec3 color = vec3(1.0);
  ...
  color = l15_alternate(pos, color, time, audio);
  color = 1.0 - color;
