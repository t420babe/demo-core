...
#endif
mat2 abh_80_blob_m(float a){
  float c=tan(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float abh_80_blob(vec3 p3, float time){
    p3.xz *= abh_80_blob_m(time * 0.4);
    p3.xy*= abh_80_blob_m(time * 0.3);
    vec3 q = p3 * 2.0 + time;
    float x0 = length( p3 );
...
void abh_80(vec3 p3, float time, peakamp audio) {
  ...
  float rz = abh_80_blob(p3, time);
  float f = ( rz * abh_80_blob(p3, y) ) * 1.0;
  // color = (m) * color * m * vec3(1.0);
  vec3 l = vec3(sin(time), tan(time), 0.5) + vec3(abs(audio.lowpass), abs(audio.bandpass), abs(audio.notch)) * f * sin(time);
  color = l;
