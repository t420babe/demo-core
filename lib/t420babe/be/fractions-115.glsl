...
float fractions_115_map(vec3 p3, float time) {
  ...
  float x2 = fract(p3.x - (10.0 * q.y + (q.x / q.y) ) ) * 10.0 * audio.notch;
  return x0 * x1 + x2 * 5.0;
}

void fractions_115(vec3 p3, float time, peakamp audio) {
  ...
  float rz = fractions_115_map(p3, time);
  ...
  float f =  ( rz - fractions_115_map(p3 * 1.0, wrap_time(time, 10.0)) ) ;
