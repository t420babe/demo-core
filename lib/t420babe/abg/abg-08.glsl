...
void abg_08(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  // pos *= (abs(sin(time * 0.5))) * 500.0;
  pos *= wrap_time(time, 800.0) + 100.0;
  // pos *= 500.0;
  ...
  gl_FragColor -= texture2D(u_fb, vec2(abs(cos(p3.yx/ (PI * 0.60) + PI)) + 0.10) + vec2(0.001, 0.001)) - 0.002;
}
#endif
