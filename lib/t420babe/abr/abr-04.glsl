...
void abr_04(vec3 p3, float time, peakamp audio) {
  vec2 uv = p3.xy * 2.0 * (sin(time * 0.1) + 1.0);

  // rate
  float alt_time = 2.0 * time;

  float height = 0.0;
  // float height = 1. + cos(alt_time);
  float dist = 5.0;
  vec3 ro = vec3(dist * cos(alt_time), sin(time), dist * sin(alt_time));
  ...
  col *= audio.notch * 30.0;
  col.r *= audio.bandpass;
  col.g *= audio.highpass;
  col.b *= audio.lowpass;
  // col += audio.lowpass;
