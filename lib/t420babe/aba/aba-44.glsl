...
vec3 aba_44_alternate(in vec2 pos, vec3 color, float time, peakamp audio) {
  ...
  // float r = 1.1 * (audio.highpass * audio.lowpass + (audio.notch));
...
void aba_44(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  vec3 color = vec3(1.0);
  float mul = 1.0;
  audio.lowpass   *= mul;
  ...
  color = aba_44_alternate(pos, color, time, audio);
  // color = 1.0 - color;
