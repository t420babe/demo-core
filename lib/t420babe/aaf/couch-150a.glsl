...
#define T420BABE_COUCH_150A
float couch150a_random (in vec2 st) {
...
float couch150a_noise (in vec2 st, peakamp audio) {
    // vec2 i = log(st.xx *  0.00001);
    vec2 i = (tan(st.xx *  0.00001));
    vec2 f = fract((st.yy * 0.1));

    // Four corners in 2D of a tile
    float a = couch150a_random(i);
    float b = couch150a_random(i * f + vec2(1.0, 10.0 * audio.bandpass));
    float c = couch150a_random(i * f + vec2(0.0, 1.0));
    float d = couch150a_random(i * f + vec2(1.0, 1.0));
...
float couch150a_fbm (in vec2 st, peakamp audio) {
    st.y += 2.5;
    // st /= 10.0;
    // Initial values
    ...
    for (int i = 0; i < octaves; i++) {
        value += amplitude * couch150a_noise(st, audio);
        st *= 8.0;
...
void couch150a(vec2 pos, float u_time, peakamp audio, out vec3 color) {
    // pos.y += sin(u_time * 0.5);
    // pos.y += 0.5;
    color += couch150a_fbm(pos * 5.0, audio);
