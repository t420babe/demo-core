...
float abe_21_map(vec3 pos, float time){
...
void abe_21(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  // 365.0
  ...
  for(int i = 0; i <= 7; i++)	{
    ...
    float rz = abe_21_map(pos, time);
    float dim = 1.0;
    float f = clamp( ( rz - abe_21_map(pos + 0.5, wrap_time(time, 10.0)) ) * dim, 0.05, 10.0 );
    float r_mul = 0.20;
    float g_mul = 2.0;
    float b_mul = 1.5;
    ...
    vec3 l = vec3(0.35, 0.1, 0.3) + vec3(abs(audio.bandpass) * r_mul, abs(audio.bandpass) * g_mul, abs(audio.notch) * b_mul) * atan(f * pos.y * 0.1) + abs(audio.notch) / 0.9;
    color *= l;
