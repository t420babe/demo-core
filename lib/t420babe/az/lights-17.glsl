...
#ifndef T420BABE_LIGHTS_17
#define T420BABE_LIGHTS_17
...
vec3 alternate(in vec2 pos, vec3 color, peakamp audio) {
  ...
  // float mul = abs(sin(u_time)) * 0.5;
  float mul = (clamp(sin(u_time * 0.5), 0.5, 1.0) + 0.05) * 0.4;
...
vec3 lights_17(vec2 pos, float u_time, peakamp audio) {
  vec3 color = vec3(1.0);
  float mul = 0.65;
  audio.lowpass   *= mul;
  audio.highpass  *= mul;
