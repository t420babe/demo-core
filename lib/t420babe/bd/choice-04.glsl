...
#ifndef T420BABE_CHOICE_04
#define T420BABE_CHOICE_04
...
float choice04_map(vec3 pos, float time){
...
vec3 choice_04(vec2 pos, float time, peakamp audio) {
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.0;
  audio.highpass  *= 1.0;
  audio.bandpass  *= 1.0;
  ...
  for(int i = 0; i <= 10; i++)	{
    ...
    float rz = choice04_map(pos, time);
    float f = clamp( ( rz - choice04_map(pos + 0.5, time) ) * 0.5, 0.1, 2.0 );
    float r_mul = 1.0;
    float g_mul = 1.5;
    float b_mul = 1.5;
    vec3 l = vec3(0.5, 0.5, 0.5) + vec3(abs(audio.lowpass) * r_mul, abs(audio.bandpass) * b_mul, abs(audio.highpass) * g_mul) * f;
    // color -= l * 0.2;
    color *= l;
