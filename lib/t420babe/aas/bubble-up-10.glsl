// #effect #trippy #fav2 #shadershoot
#ifndef T420BABE_BUBBLE_UP_10
#define T420BABE_BUBBLE_UP_10
...
vec2 cellular2x2(vec2 P) {
  ...
  vec4 ox = mod(p, abs(2.0 * audio.notch)) * K2;
...
void bubble_up_10(vec2 pos, float u_time, peakamp audio, inout vec3 color) {
  float inv = 1.0;
  float zoom = 20.0 * inv;
  // pos.y += 0.20 * inv;
  // pos.y -= 0.20;
  ...
  color.r *= abs(tan(n * sin(n / u_time * 0.5)));
  color.b *= abs(sin(n * tan(u_time * 1.0) + 3.14));
  // color = 1.0 - color;
  // color = color.rbg;
  // color = color.bgr;
