...
float abe_13_map(vec3 pos, float time){
...
void abe_13(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  // 365.0
  ...
  for(int i = 0; i <= 7; i++)	{
    ...
    float rz = abe_13_map(pos, time);
    float dim = 1.0;
    float f = clamp( ( rz - abe_13_map(pos + 0.5, wrap_time(time, 10.0)) ) * dim, 0.5, 5.0 );
    float r_mul = 0.1;
    float g_mul = 2.0;
    float b_mul = 1.5;
