#ifndef T420BABE_NYC2020BB_01
#define T420BABE_NYC2020BB_01
...
vec3 nyc2020bb_01(vec2 pos, float u_time, peakamp audio) {
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.0;
  audio.highpass  *= 1.0;
  audio.bandpass  *= 1.0;
  ...
  // n = step(n, sin(pos.x));
  color = vec3(n);
  color /= spiral_pxl(pos * 3.0 * clamp(abs(audio.notch), 0.2, 1.0), wrap_time(u_time, 10.0) + 5.0);
