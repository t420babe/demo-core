...
float choice_32_map(vec3 pos, float time, peakamp audio){
...
vec3 choice_32(vec2 pos, float time, peakamp audio) {
  // 365.0
  ...
  for(int i = 0; i <= 2; i++)	{
    ...
    float rz = choice_32_map(pos, time, audio);
    float dim = 1.0;
    // float f = clamp( ( rz - choice_32_map(pos * audio.notch * 3.0, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );

    // float f = clamp( ( rz - choice_32_map(pos + 0.5, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );
    float f = clamp( ( rz - choice_32_map(abs(sin(pos)) * audio.lowpass * 4.0, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );
