...
float abe04_map(vec3 pos, float time){
...
vec3 abe_04(vec2 pos, float time, peakamp audio) {
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.0;
  audio.highpass  *= 1.0;
  audio.bandpass  *= 1.0;
  ...
  for(int i = 0; i <= 10; i++)	{
    ...
    float rz = abe04_map(pos, time);
    float f = clamp( ( rz - abe04_map(pos + 0.5, time) ) * 0.5, 0.1, 2.0 );
    float r_mul = 1.0;
    float g_mul = 1.5;
    float b_mul = 1.5;
