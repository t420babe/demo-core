// #effect #effectmisx_11_shape #fav4 #shadershoot #corey
#ifndef T420BABE_MISX_11
#define T420BABE_MISX_11
...
float misx_11_shape(vec2 pos, float radius, float u_time, peakamp audio) {
...
float misx_11_shape_border(vec2 pos, float radius, float width, float u_time, peakamp audio) {
  return misx_11_shape(pos, radius, u_time, audio) - misx_11_shape(pos, radius - width, u_time, audio);
}
...
vec3 misx_11(vec2 pos, float u_time, peakamp audio) {
  ...
  // misx_11_shape_color_border(pos, 1.0, 0.10, u_time, audio, color);

  // Color 0
  // color.b += audio.lowpass * 2.0;
  color *= misx_11_shape_border(pos, 1.0, 0.10, u_time, audio);
  color.g *= audio.lowpass * 2.0;

  // // Color 0
  // color.g += audio.lowpass * 2.0;
  // color *= misx_11_shape_border(pos, 1.0, 0.01, u_time, audio);
  // color.g *= audio.lowpass * 2.0;
