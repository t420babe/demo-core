...
#define T420BABE_COUCH_7
float couch7_random (in vec2 st) {
...
float couch7_noise (in vec2 st, peakamp audio) {
  vec2 i = floor(st);
  vec2 f = fract(st);

  // Four corners in 2D of a tile
  float a = couch7_random(i);
  float b = couch7_random(i + vec2(1.0, audio.bandpass));
  float c = couch7_random(i + vec2(0.0, 1.0));
  float d = couch7_random(i + vec2(1.0, 1.0));
...
float couch7_fbm (in vec2 st, peakamp audio) {
  ...
  for (int i = 0; i < octaves; i++) {
    value += amplitude * couch7_noise(st, audio);
    st *= 8.0;
