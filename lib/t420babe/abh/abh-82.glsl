...
#endif
mat2 abh_82_blob_m(float a){
  float c=cos(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float abh_82_blob(vec3 p3, float time){
    p3.xz *= abh_82_blob_m(time * 0.4);
    p3.xy*= abh_82_blob_m(time * 0.3);
    vec3 q = p3 * 2.0;
    float x0 = length( p3 );
...
void abh_82(vec3 p3, float time, peakamp audio) {
  time += t2s(0, 6, 13);
  // audio.lowpass   *= 2.0;
  // audio.highpass  *= 2.0;
  // audio.bandpass  *= 2.0;
  ...
  p3.x -= 0.5;
  float rz = abh_82_blob(p3, time);
  float f = ( rz * abh_82_blob(p3, y) ) * 10.0;
