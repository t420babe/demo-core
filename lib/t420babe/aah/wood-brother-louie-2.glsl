...
#define T420BABE_WOOD_BROTHER_LOUIE_2
// 22c935e, 23:46
...
float wbl2_noise(vec2 pos, float u_time) {
  vec2 i = floor(pos);
  vec2 f = fract(pos);
  vec2 u = f*f*(3.0-2.0*f);
  return mix( mix( wbl2_random( i + vec2(0.0,0.0) ),
        wbl2_random( i + vec2(1.0,0.0) ), u.x),
        mix( wbl2_random( i + vec2(0.0,1.0) ),
        wbl2_random( i + vec2(1.0,1.0) ), u.x), u.y);
}

mat2 wbl2_rotate2d(float angle){
  return mat2(cos(angle),-sin(angle),
      sin(angle),cos(angle));
}

float wbl2_lines(in vec2 pos, float b){
  float scale = 10.0;
  pos *= scale;
...
void wbl2_wood(vec2 pos, float u_time, peakamp audio, out vec3 color) {
  ...
  // Add noise
  pos2 = wbl2_rotate2d( noise(pos2, u_time) ) * pos2;

  // Draw lines
  pattern = wbl2_lines(pos2,0.1);
