// Parallel Lines - Enzo Bennet Remix by ATTLAS, Enzo Bennet
#ifndef T4B_ABF_116
#define T4B_ABF_116
...
void abf_116(vec3 p3, float time, peakamp audio) {
  ...
  vec3 raw_p3 = p3;
  float raw_time = time;
  time = wrap_time(time * 1.0, 60.0) + 10.0;
  // time *= 0.5;
  vec3 color = vec3(1.0);
  vec3 p3_m1 = p3;
  p3_m1.xz *= rotate2d(time * 0.5);
  p3_m1.yx *= rotate2d(time * 0.5);
  ...
  float rz = abf_116_map(p3, raw_time);
  ...
  vec3 l = vec3(2.0 * audio.notch) * sin(0.1 * f ) + cos( time * f ) / f;
