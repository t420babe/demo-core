...
float abe_30_map(vec3 pos, float time){
...
void abe_30(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  // 365.0
  ...
  for(int i = 0; i <= 1; i++)	{
    ...
    float rz = abe_30_map(pos, time);
    float dim = 1.0;
    float f = clamp( ( rz - abe_30_map(pos + 0.5, time * 1.5) ) * dim, 0.5, 5.0 );
    float r_mul = 1.0;
    float g_mul = 1.0;
    float b_mul = 1.0;
