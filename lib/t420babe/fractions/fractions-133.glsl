// #katie1
#ifndef T4B_FRACTIONS_133
#define T4B_FRACTIONS_133
...
float f_133_plot(vec2 p2, float m) {
  return smoothstep(m - 0.15, m, p2.y) - smoothstep(m, m + 0.15, p2.y);
}

void fractions_133(vec3 p3, float time, peakamp audio) {
  // time += 100.0;
  vec3 color = vec3(1.0);
  // p3 *= time * 0.5;
  // p3 *= 3.0;
  ...
  p3.xz *= rotate2d(time * 2.1 + 0.0);
  p3.xy *= rotate2d(time * 2.1);

  float y = 1.0 * (tan(p3.x * 10.0) + cos(p3.x * time));
  // float m = f_133_plot(vec2(p3.x, p3.y), y) * 1.0;
