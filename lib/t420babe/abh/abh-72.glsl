...
#endif
mat2 abh_72_blob_m(float a){
  float c=cos(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float abh_72_blob(vec3 p3, float time){
    p3.xz *= abh_72_blob_m(time * 0.4);
    p3.xy*= abh_72_blob_m(time * 0.3);
    vec3 q = p3 * 2.0 + time;
...
void abh_72(vec3 p3, float time, peakamp audio) {
  ...
  float rz = abh_72_blob(p3, time);
  float f = ( rz / abh_72_blob(p3, y) ) * 10.0;
