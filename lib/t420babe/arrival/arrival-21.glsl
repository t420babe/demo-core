...
// #fav5 #roli #katie1
#ifndef T4B_ARRIVAL_21
#define T4B_ARRIVAL_21
...
void arrival_21(vec3 p3, float time, peakamp audio) {
  time *= 0.5;
  // p3 *= 0.9;
  audio.notch     *= 1.0;
  audio.bandpass  *= 1.0;
 ...
 // bri /= (rhombus) * 1.0 * audio.notch;
  ...
  gl_FragColor = vec4(1.0 - 100.0 * color.rgb, bri);
  ...
  gl_FragColor *= texture2D(u_fb, vec2(p3.yx/ 2.0 + 0.5) - vec2(0.00, 0.0001)) - 0.080 * sin(p3.x);
}
