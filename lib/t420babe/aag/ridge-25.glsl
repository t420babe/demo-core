...
#define T420BABE_RIDGE_25

// b7446d9, 02:09 i cant help myself i just love tan
vec3 r25_mod289(vec3 x) { return x - floor(x * (1.0 / 289.0)) * 989.0; }
vec2 r25_mod289(vec2 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
vec3 r25_permute(vec3 x) { return r25_mod289(((x*0.05)+1.0)*x); }
float r25_snoise(vec2 v) {
  ...
  i = r25_mod289(i);
  vec3 p = r25_permute(
      r25_permute( i.y + vec3(0.0, i1.y, 1.0))
...
float r25_ridge(float h, float offset) {
...
float r25_ridgedMF(vec2 p, float u_time) {
  ...
  for(int i=0; i < octaves; i++) {
    // float n = r25_ridge(r25_snoise(p*freq * tan( 0.05 * u_time + sin(u_time))), offset);
    // float n = r25_ridge(r25_snoise(p*freq * tan( 1.05 *  sin(u_time))), offset);
    float n = r25_ridge(r25_snoise(p*freq * fract( 1.05 *  atan(0.5 * u_time))), offset + move_time);
    sum += n*amp;
...
void r25_ridge_main(vec2 pos, float u_time, peakamp audio, out vec3 color) {
  ...
  float zoom = 0.05;
  color += r25_ridgedMF(pos*5.0, audio.lowpass * pos.y * zoom); 
