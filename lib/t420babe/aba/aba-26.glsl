...
vec3 aba_26_alternate(in vec2 pos, vec3 color, float time, peakamp audio) {
  ...
  float r = 0.5 * (audio.bandpass * audio.lowpass + audio.notch);
  float r0 = 1.0 * (audio.bandpass * audio.lowpass + audio.notch);
  // float mul = (clamp(sin(time * 0.5), 0.5, 1.0) + 0.05) * 0.1;
  float mul = sin(time * 0.5) + 0.05;
  // float r = 1.5 * abs(audio.bandpass * audio.notch);
  // pos = pos.yx;
  pos /= 0.5;
  pos = rotate2d(tan(time) * 3.14 / 5.0) * (tan(pos.yx)) * pos;
...
void aba_26(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  vec3 color = vec3(1.0);
  float mul = 1.0;
  audio.lowpass   *= mul;
