...
float abe_43_map(vec3 pos, float time, peakamp audio){
...
void abe_43(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  // 365.0
  ...
  for(int i = 0; i <= 2; i++)	{
    ...
    float rz = abe_43_map(pos, time, audio);
    float dim = 1.0;
    // float f = clamp( ( rz - abe_43_map(pos * audio.notch * 3.0, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );

    // float f = clamp( ( rz - abe_43_map(pos + 0.5, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );
    float f = clamp( ( rz - abe_43_map(abs(sin(pos * time * 0.5)) * audio.lowpass * 4.0, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );
