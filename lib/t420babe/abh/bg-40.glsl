...
#endif
mat2 bg_40_rotate2d(float a){
  float c=cos(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float bg_40_blob(vec3 p3, float time){
  p3 = p3.yxz;
...
void bg_40(vec3 p3, float time, peakamp audio) {
  // audio.lowpass = 0.05;
  // audio.highpass = 0.05;
  // audio.bandpass = 0.05;
  // audio.notch = 0.05;
  ...
  // p3.x -= 0.5;
  float rz = bg_40_blob(p3, time);
  float f = ( rz * bg_40_blob(p3, y) ) * 1.0;
