...
#ifndef T420BABE_LIGHTS_20
#define T420BABE_LIGHTS_20
...
vec3 alternate(in vec2 pos, vec3 color, peakamp audio) {
  ...
  pos = abs(tan(pos * 1.3) * (wrap_time(u_time, 4.0) + 4.5));
  ...
  pos = rotate2d(cos(u_time * 3.0) * 3.14 / 1.0) * pos;
  pos /= 3.5;
...
vec3 lights_20(vec2 pos, float u_time, peakamp audio) {
  vec3 color = vec3(1.0);
  float mul = 1.0;
  audio.lowpass   *= mul;
  audio.highpass  *= mul;
