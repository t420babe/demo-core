...
#ifndef T420BABE_DAY_0_21
#define T420BABE_DAY_0_21
...
vec3 day_0_21(vec2 pos, float u_time, peakamp audio) {
  ...
  // pos = pos.yx;
  pos *= 5.5;
  // pos = rotate2d(tan(u_time) * 3.14 / 1.0) * pos;
  pos.y += 1.0;
  pos.y *= 1.5 * sin(pos.y);
  pos.x *= 5.5 * sin(pos.y);
  ...
  // color -= 1.5;
  ux.y += 1.5;
  color *= 2.0 * shapes(ux * 20.0, u_time, audio);
  color /= sharp(circle_sdf(pos));
  return color;
}
#endif
