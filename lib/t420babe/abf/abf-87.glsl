...
float abf_87_map(vec3 p3, float time) {
...
void abf_87(vec3 p3, float time, peakamp audio) {
  audio.lowpass *= 2.0;
  audio.highpass *= 2.0;
  audio.bandpass *= 2.0;
  audio.notch *= 2.0;
  ...
  // float f = clamp( ( rz / abf_87_map(p3 + 0.5, wrap_time(time, 10.0)) ) * 1.0, 0.05, 10.0 );
  float f = rz / abf_87_map(p3 + 0.5, wrap_time(time, 10.0));
