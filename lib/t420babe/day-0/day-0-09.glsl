...
#ifndef T420BABE_DAY_0_09
#define T420BABE_DAY_0_09
...
vec3 day_0_09(vec2 pos, float u_time, peakamp audio) {
  vec3 color = vec3(1.0);
  ...
  // n = step(n, sin(pos.x));
  // color = vec3(n / (5.0 * audio.notch));
  // color = vec3(20.0 * audio.bandpass);
  color = vec3(n);
  // pos = (pos.yx);
  // pos *= 50.0;
  pos.x *= 45.5 * abs(sin(pos.y));
  // pos.y += abs(audio.notch) * 5.0;
  pos.x *= 5.5 * abs(sin(pos.y));
