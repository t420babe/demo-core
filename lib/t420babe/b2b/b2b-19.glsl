...
#endif
mat2 b2b_19_blob_m(float a){
  float c=cos(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float b2b_19_blob(vec3 p3, float time){
...
void b2b_19(vec3 p3, float time, peakamp audio) {
  // audio.lowpass = 0.05;
  // audio.highpass = 0.05;
  // audio.bandpass = 0.05;
  // audio.notch = 0.05;
  ...
  p3.x -= 0.5;
  float rz = b2b_19_blob(1.0 * p3, time);
  float f = ( rz / b2b_19_blob(p3, y) ) * 10.0;
