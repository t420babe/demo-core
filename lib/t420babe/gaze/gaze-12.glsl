// #effect #shadershoot
#ifndef T420BABE_GAZE_12
#define T420BABE_GAZE_12
...
float hexagon_sdf(vec2 st, float size, float freq) {
  st = abs(st / 4.0);
  st /= size;
  ...
  // hexagon =  max(abs(st.y), st.x * 0.866025 + st.y * 0.5);
  // hexagon =  max((st.x), st.x * (freq * 3.0) - st.y * (freq) * 1.0);
  // hexagon =  max((st.x), st.y * (freq * 3.0) - (st.x - st.y) / freq);
  // hexagon =  min((st.y), st.y * (freq * 3.0) - (st.x - st.y) / freq);
  // hexagon =  max((st.y), st.y * (freq * 3.0) - (st.x - st.y) / freq);
  //
  hexagon =  max((st.x) * 0.5, st.y * (freq * 3.0) - (st.x - st.y) / freq);
  // hexagon =  st.x * (freq * 3.0) - (st.x - st.y) / freq;
  // hexagon /= st.x;
...
void gaze_12(vec2 pos, float u_time, peakamp audio, inout vec3 color) {
  audio.lowpass   *= 0.4;
  audio.highpass  *= 1.5;
  audio.bandpass  *= 0.5;
  audio.notch     *= 1.5;
  ...
  float hex = sharp(hexagon_sdf(pos, 5.0, audio.notch));
  color /= hex;
