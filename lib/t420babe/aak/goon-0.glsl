...
void goon_0_1(vec2 pos, float u_time, peakamp audio, out vec3 color) {
  color = vec3(0.0);
  // color = alligator
  ...
  float my_hex = hexagon_web(pos);
  color /= my_hex;
  // float my_tri = triandl
  // color.b *= audio.lowpass * 0.5;
  // color.b *= audio.lowpass * abs(sin(u_time));
  color.b += audio.bandpass * abs(cos(u_time));
  // color.g *= audio.highpass * 0.5;
  color.b *= audio.highpass * abs(tan(u_time));
  // color.g *= audio.lowpass * abs(sin(u_time));
  // color += my_hex;
  green_concentric_0(pos, u_time, audio, purp_color);
  color *= purp_color;
