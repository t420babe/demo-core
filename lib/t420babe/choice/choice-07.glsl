...
#ifndef T420BABE_CHOICE_07
#define T420BABE_CHOICE_07
...
float choice07_map(vec3 pos, float time){
...
vec3 choice_07(vec2 pos, float time, peakamp audio) {
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.0;
  audio.highpass  *= 1.0;
  audio.bandpass  *= 1.0;
  ...
  for(int i = 0; i <= 10; i++)	{
    ...
    pos *= 7.0;
    float rz = choice07_map(pos, time);
    float f = clamp( ( rz - choice07_map(pos + 0.5, time) ) * 0.5, 0.1, 2.0 );
    float r_mul = 0.2;
    float g_mul = 1.8;
    float b_mul = 2.0;
