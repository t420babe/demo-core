...
float b2b_10_map(vec3 p3, float time){
...
void b2b_10(vec3 p3, float time, peakamp audio) {
  time += 8.0;
  vec3 color = vec3(1.0);
  p3 *= time * 0.05;
  // p3.y += 0.5;
  ...
  float y = (tan(2.0 * p3.x) + cos(log(p3.y) * p3.y * time));
  float m = plot(vec2(p3), y, 0.05) * 5.0;
  float rz = b2b_10_map(p3, time);
