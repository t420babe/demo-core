...
#endif
mat2 b2b_26_blob_m(float a){
  float c=cos(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float b2b_26_blob(vec3 p3, float time){
  vec3 p3_raw = p3;
  p3.xz *= blob_m(time * 0.4);
  p3.xy*= blob_m(time * 0.3);
  vec3 q = p3 * 2.0 + time;
  float x0 = length( p3.zyy + vec3( sin(time * 0.7) ) );
...
void b2b_26(vec3 p3, float time, peakamp audio) {
  // audio.lowpass   *= 2.0;
  // audio.highpass  *= 2.0;
  // audio.bandpass  *= 2.0;
  // audio.notch     *= 2.0;
  ...
  p3.x -= 0.5;
  float rz = b2b_26_blob(p3, time);
  float f = ( rz / b2b_26_blob(p3, y) ) * 10.0;
