...
#define T420BABE_IN_SEARCH_OF_04
...
vec2 iso_04_cellular2x2x2(vec3 P) {
...
float iso_04_spiral(vec2 st, float t) {
...
vec3 iso_04(vec2 pos, float u_time, peakamp audio) {
  u_time +=100.0;
  vec3 color = vec3(1.0);
  audio.lowpass   *= 0.5;
  audio.highpass  *= 0.5;
  ...
  st *= 25.0 * abs(sin(u_time * 0.01));
	vec2 F = iso_04_cellular2x2x2(vec3(st * 1.0, u_time));
	float n = smoothstep(0.0, abs(sin(u_time * 0.05)) + 1.0, 1.0 * F.x) / ( abs(audio.notch * 0.5));
  // n = step(n, sin(pos.x));
  color = vec3(n);
  color -= iso_04_spiral(pos.yx * 5.0 * abs(audio.bandpass * 1.0), wrap_time(u_time, 10.0) + 10.0);
