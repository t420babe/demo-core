...
vec2 day_03_cellular2x2x2(vec3 P) {
...
float day_03_spiral(vec2 st, float t) {
    float r = dot(st.yx, st.yx) * 0.5;
    float a = atan(st.y,st.x)  * 0.5;
    return abs(((sin(r * t)   / r)));
}

vec3 day_03_shapes(vec2 pos, float u_time, peakamp audio) {
    float x = pos.x * (sin(u_time));
    float y = pos.y * cos(u_time);
...
vec3 day_03(vec2 pos, float u_time, peakamp audio) {
  vec3 color = vec3(1.0);
  audio.lowpass   *= 1.0;
  audio.highpass  *= 1.0;
  audio.bandpass  *= 1.0;
  ...
  st.y += 1.0;
  st *= 20.0;
	vec2 F = day_03_cellular2x2x2(vec3(st * 1.0, u_time));
  ...
  pos.y *= 1.5 * sin(pos.y);
  pos.x *= 5.5 * sin(pos.y);
  color -= abs(sin(u_time * 0.1) + 2.0) + 5.0 * day_03_spiral(abs(sin(pos.yy) * cos(pos.xy)) * 4.5 * abs(1.0 * audio.bandpass), 0.1 * wrap_time(u_time, 10.0) + 10.0);
	color -= day_03_spiral(3.0 * pos.yx * abs(audio.bandpass), 1.0 * wrap_time(u_time, 10.0) + 10.0);
  ...
  // color -= 1.5;
  color *= 1.0 * day_03_shapes(ux.yx * 20.0, u_time, audio);


  color *= vec3(1.4, 0.834, 1.9234);
  return color;
}
#endif
