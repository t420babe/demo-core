...
float fractions_64_map(vec3 p3, float time) {
...
void fractions_64(vec3 p3, float time, peakamp audio) {
  ...
  float rz = fractions_64_map(p3, time);
  ...
  float f =  ( rz - fractions_64_map(p3 * 1.0, wrap_time(time, 10.0)) ) ;
