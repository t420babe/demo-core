...
float abe_41_map(vec3 pos, float time, peakamp audio){
...
void abe_41(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  // 365.0
  ...
  for(int i = 0; i <= 3; i++)	{
    ...
    float rz = abe_41_map(pos3, time, audio);
    float dim = 1.0 * (sin(time * 0.5));
    // float f = clamp( ( rz - abe_41_map(pos * audio.notch * 3.0, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );

    // float f = clamp( ( rz - abe_41_map(pos + 0.5, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );
    float f = clamp( ( rz - abe_41_map(abs(sin(pos3 * 5.0)) * audio.lowpass * 4.0, time, audio) ) * dim, 0.1, 5.0 );
