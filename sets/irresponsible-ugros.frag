...
void main() {
  ...
  float tri = circle_1(0.7 * pos, 0.5);

  vec3 n_color;
  float n = cellular_3d(3.5 * pos, u_time, audio, n_color);
  // say_nothing_none(sin(0.1 * u_time + pos.yx), u_time, audio, color);
  say_nothing_none(10.0 * pos, u_time, audio, color);
  // color /= n + 0.00;
  // color.r /= n + 0.00;
  // color.g /= n + 0.10;
  color.b *= 0.10;
