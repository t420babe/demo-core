...
float abe53_map(vec3 pos, float time){
...
void abe_53(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  // 365.0
  ...
  for(int i = 0; i <= 10; i++)	{
    ...
    float rz = abe53_map(pos, time);
    float dim = 1.0;
    float f = clamp( ( rz - abe53_map(pos + 0.5, wrap_time(time, 10.0)) ) * dim, 0.5, 5.0 );
    float r_mul = 0.1;
    float g_mul = 2.0;
    float b_mul = 1.5;
    ...
    if (time - offset < t2s(0.0, 1.0, 35.0)) {
      ...
      color.b *= 1.8;
      color.g *= 0.85;
    } else if (time - offset >= t2s(0.0, 3.0, 23.0) && time - offset < t2s(0.0, 4.0, 24.0)) {
      ...
      // #:56
    } else if (time - offset >= t2s(0.0, 4.0, 24.0) && time - offset < t2s(0.0, 6.0, 26.0)) {
      color = color.gbr;
      color = color.grb;
      color.r *= 0.6;
      color.b *= 1.8;
