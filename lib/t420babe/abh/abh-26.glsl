...
#endif
mat2 abh_26_blob_m(float a){
  float c=cos(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float abh_26_blob(vec3 p3, float time){
  vec3 p3_raw = p3;
...
void abh_26(vec3 p3, float time, peakamp audio) {
  // audio.lowpass   *= 2.0;
  // audio.highpass  *= 2.0;
  // audio.bandpass  *= 2.0;
  // audio.notch     *= 2.0;
  ...
  p3.x -= 0.5;
  float rz = abh_26_blob(p3, time);
  float f = ( rz / abh_26_blob(p3, y) ) * 10.0;
