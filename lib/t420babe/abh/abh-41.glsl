...
#endif
mat2 abh_41_rotate2d(float a){
  float c=cos(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float abh_41_blob(vec3 p3, float time){
  p3 = p3.yxz;
...
void abh_41(vec3 p3, float time, peakamp audio) {
  // audio.lowpass = 0.05;
  // audio.highpass = 0.05;
  // audio.bandpass = 0.05;
  // audio.notch = 0.05;
  ...
  // p3.x -= 0.5;
  float rz = abh_41_blob(p3, time);
  float f = ( rz * abh_41_blob(p3, y) ) * 1.0;
