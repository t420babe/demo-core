...
#endif
mat2 abh_69_blob_m(float a){
  float c=cos(a), s=sin(a);
  return mat2(c,-s,s,c);
}

float abh_69_blob(vec3 p3, float time){
    p3.xz *= abh_69_blob_m(time * 0.4);
    p3.xy*= abh_69_blob_m(time * 0.3);
    vec3 q = p3 * 2.0 + time;
...
void abh_69(vec3 p3, float time, peakamp audio) {
  ...
  // p3.x -= 0.5;
  float rz = abh_69_blob(1.0 * p3, time);
  float f = ( rz / abh_69_blob(p3, y) ) * 10.0;
