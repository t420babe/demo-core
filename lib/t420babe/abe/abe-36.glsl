...
float abe_36_map(vec3 pos, float time, peakamp audio){
...
void abe_36(vec3 p3, float time, peakamp audio) {
  vec2 pos = p3.xy;
  // 365.0
  ...
  for(int i = 0; i <= 2; i++)	{
    ...
    float rz = abe_36_map(pos3, time, audio);
    float dim = 1.0;
    // float f = clamp( ( rz - abe_36_map(pos * audio.notch * 3.0, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );

    // float f = clamp( ( rz - abe_36_map(pos + 0.5, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );
    float f = clamp( ( rz - abe_36_map(abs(sin(pos3 * 1.0)) * audio.lowpass * 4.0, wrap_time(time, 10.0), audio) ) * dim, 0.5, 5.0 );
