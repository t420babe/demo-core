...
float abf_88_map(vec3 p3, float time) {
...
void abf_88(vec3 p3, float time, peakamp audio) {
  audio.lowpass *= 2.0;
  audio.highpass *= 2.0;
  audio.bandpass *= 2.0;
  audio.notch *= 2.0;
  ...
  float rz = abf_88_map(p3, time);
  float y = 0.5 * abs(sin(p3.x + 1.0) + sin(p3.x * time));
  float m = plot(vec2(p3.x, p3.y), y, 1.50) * 1.0;
  float r_mul = 0.1;
  float g_mul = 2.0;
  float b_mul = 1.5;
  // float f =rz - abf_88_map(p3, wrap_time(time, 10.0));
  float f =rz - (abf_88_map(p3, sin(time* 1.0)) * 5.0);
  vec3 l = vec3(y);
  color *= l;
